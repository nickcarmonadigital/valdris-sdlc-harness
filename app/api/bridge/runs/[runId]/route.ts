...
function detailBridgePath(request: Request, runId: string) {
  ...
  const unsupported = [...new Set([...searchParams.keys()].filter((name) => !DETAIL_QUERY_NAMES.has(name)))];
  if (unsupported.length) return { ok: false as const, unsupported };
  const query = searchParams.toString();
  return { ok: true as const, path: `/runs/${encodeURIComponent(runId)}${query ? `?${query}` : ""}` };
}

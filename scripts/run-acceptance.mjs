...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === "--repo") args.repo = valueFor(index++, "--repo");
      ...
      args.sourceCommit = valueFor(index++, "--source-commit");
...
function git(repo, args, options = {}) {
  ...
  if (result.status !== 0)
    throw new Error(
      `git ${args.join(" ")} failed: ${(result.stderr || result.stdout || "unknown Git error").trim()}`,
    );
...
function assertAllowedArtifactPath(relativePath) {
  ...
  if (
    ...
    relativePath
      .split("/")
      .some((component) => Buffer.byteLength(component, "utf8") > 160)
  ) {
    throw new Error(
      `artifact bundle path is too long for portable hydration: ${relativePath}`,
    );
  }
  const [root] = relativePath.split("/");
  if (!EVIDENCE_NAMESPACE_SET.has(root))
    throw new Error(
      `artifact bundle path is outside the evidence namespaces: ${relativePath}`,
    );
}
...
function walkBundle(root) {
  ...
  while (pending.length > 0) {
    ...
    if (entriesSeen > MAX_BUNDLE_ENTRIES)
      throw new Error(
        `artifact bundle exceeds the ${MAX_BUNDLE_ENTRIES}-entry traversal limit`,
      );
    for (const entry of entries) {
      ...
      if (stats.isSymbolicLink())
        throw new Error(
          `artifact bundle must not contain symbolic links: ${relativePath}`,
        );
      if (stats.isDirectory()) {
        if (depth + 1 > MAX_BUNDLE_DEPTH)
          throw new Error(
            `artifact bundle exceeds the ${MAX_BUNDLE_DEPTH}-directory depth limit`,
          );
        pending.push({ directory: target, depth: depth + 1 });
      } else if (stats.isFile()) files.push(relativePath);
      else
        throw new Error(
          `artifact bundle must contain only directories and regular files: ${relativePath}`,
        );
    }
  }
...
function readAndValidateManifest(bundleRoot, expectedSourceCommit) {
  ...
  if (!existsSync(manifestPath))
    throw new Error(`artifact bundle is missing ${MANIFEST_NAME}`);
  const stats = lstatSync(manifestPath);
  if (stats.isSymbolicLink() || !stats.isFile())
    throw new Error(`${MANIFEST_NAME} must be a regular non-symlink file`);
  if (stats.size > MAX_MANIFEST_BYTES)
    throw new Error(
      `${MANIFEST_NAME} exceeds the ${MAX_MANIFEST_BYTES}-byte limit`,
    );
  let manifest;
  try {
    manifest = JSON.parse(readFileSync(manifestPath, "utf8"));
  } catch {
    throw new Error(`${MANIFEST_NAME} must be valid JSON`);
  }
  exactKeys(
    manifest,
    ["schema", "sourceCommit", "files"],
    "artifact bundle manifest",
  );
  if (manifest.schema !== BUNDLE_SCHEMA)
    throw new Error(`artifact bundle schema must be ${BUNDLE_SCHEMA}`);
  if (!GIT_OBJECT_ID.test(manifest.sourceCommit || ""))
    throw new Error(
      "artifact bundle sourceCommit must be a lowercase full Git object ID",
    );
  if (manifest.sourceCommit !== expectedSourceCommit)
    throw new Error(
      "artifact bundle sourceCommit does not match --source-commit",
    );
  if (!Array.isArray(manifest.files) || manifest.files.length === 0)
    throw new Error("artifact bundle files must be a non-empty array");
  if (manifest.files.length > MAX_FILE_COUNT)
    throw new Error(`artifact bundle exceeds the ${MAX_FILE_COUNT}-file limit`);
  ...
  for (const [index, file] of manifest.files.entries()) {
    exactKeys(
      file,
      ["path", "sha256", "size"],
      `artifact bundle file ${index + 1}`,
    );
    assertAllowedArtifactPath(file.path);
    if (file.path <= previousPath)
      throw new Error(
        "artifact bundle files must be strictly sorted by canonical path",
      );
    previousPath = file.path;
    if (!SHA256.test(file.sha256 || ""))
      throw new Error(
        `artifact bundle digest must be lowercase SHA-256: ${file.path}`,
      );
    if (
      ...
      throw new Error(
        `artifact bundle file size is invalid or exceeds ${MAX_FILE_BYTES} bytes: ${file.path}`,
      );
    totalBytes += file.size;
    if (totalBytes > MAX_TOTAL_BYTES)
      throw new Error(
        `artifact bundle exceeds the ${MAX_TOTAL_BYTES}-byte aggregate limit`,
      );
    const collisionKey = portableCollisionKey(file.path);
    const prior = portablePaths.get(collisionKey);
    if (prior)
      throw new Error(
        `artifact bundle paths collide on a portable filesystem: ${prior} and ${file.path}`,
      );
    portablePaths.set(collisionKey, file.path);
  }
  if (!manifest.files.some((file) => file.path === "run/packet.json"))
    throw new Error("artifact bundle must contain run/packet.json");

  const inventory = walkBundle(bundleRoot).filter(
  ...
  for (const relativePath of inventory) {
    ...
    if (inventoryCollisionKeys.has(collisionKey))
      throw new Error(
        `artifact bundle inventory has a portable path collision: ${relativePath}`,
      );
  ...
  if (JSON.stringify(inventory) !== JSON.stringify(declared))
    throw new Error(
      "artifact bundle inventory must exactly match the manifest; undeclared, missing, or misordered files are forbidden",
    );
  ...
  for (const file of manifest.files) {
    ...
    if (!sourceStats.isFile() || sourceStats.isSymbolicLink())
      throw new Error(
        `artifact bundle entry must be a regular non-symlink file: ${file.path}`,
      );
    if (sourceStats.size !== file.size)
      throw new Error(
        `artifact bundle size does not match the manifest: ${file.path}`,
      );
    if (sha256(readFileSync(source)) !== file.sha256)
      throw new Error(
        `artifact bundle digest does not match the manifest: ${file.path}`,
      );
  }
  return manifest;
}
...
function assertCleanSourceCheckout(repoRoot) {
  for (const args of [
    ...
    if (result.status !== 0)
      throw new Error(
        "source checkout contains tracked changes before artifact hydration",
      );
  }
  for (const args of [
    ...
    if (output.length > 0)
      throw new Error(
        "source checkout contains pre-existing untracked or ignored files before artifact hydration",
      );
  }
}
...
function assertSafeDestination(repoRoot, relativePath) {
  ...
  if (!isWithin(repoRoot, destination))
    throw new Error(
      `artifact destination escapes the source checkout: ${relativePath}`,
    );
  if (existsSync(destination))
    throw new Error(
      `artifact hydration refuses to overwrite an existing source or validator path: ${relativePath}`,
    );
  let cursor = repoRoot;
  for (const component of relativePath.split("/").slice(0, -1)) {
    ...
    if (stats.isSymbolicLink() || !stats.isDirectory())
      throw new Error(
        `artifact hydration parent must be a real directory: ${relativePath}`,
      );
  }
  return destination;
}
...
function ensureSafeParent(repoRoot, relativePath, createdDirectories) {
  ...
  for (const component of relativePath.split("/").slice(0, -1)) {
    ...
    if (stats.isSymbolicLink() || !stats.isDirectory())
      throw new Error(
        `artifact hydration parent must remain a real directory: ${relativePath}`,
      );
  }
}
...
function hydrateArtifacts(
  ...
  const destinations = new Map(
    files.map((file) => [
      file.path,
      assertSafeDestination(repoRoot, file.path),
    ]),
  );
  try {
    for (const file of files) {
      ...
      if (
        ...
        throw new Error(
          `hydrated artifact failed its post-copy integrity check: ${file.path}`,
        );
      }
    }
  } catch (error) {
...
function assertSourceCheckoutStillBound(repoRoot, expectedHead, acceptedFiles) {
  ...
  if (currentHead !== expectedHead)
    throw new Error("source checkout HEAD changed during artifact validation");
  for (const args of [
    ["diff", "--quiet", "--", "."],
    ["diff", "--cached", "--quiet", "--", "."],
  ]) {
    ...
    if (result.status !== 0)
      throw new Error(
        "source checkout tracked state changed during artifact validation",
      );
  }
  ...
  if (
    ...
    throw new Error(
      "source checkout gained unexpected files during artifact validation",
    );
  }
}
...
function packetClosurePaths(bundleRoot, manifest, packet) {
  ...
  const pending = [];
  const add = (value) => {
    if (typeof value !== "string" || !declared.has(value) || closure.has(value))
      return;
    closure.add(value);
    pending.push(value);
  };
  for (const input of Object.values(packet.inputs || {})) add(input?.path);
  for (const artifact of Array.isArray(packet.gateArtifacts)
    ? packet.gateArtifacts
    : []) {
    add(artifact?.path);
    for (const supporting of Array.isArray(artifact?.supportingArtifacts)
      ? artifact.supportingArtifacts
      : [])
      add(supporting?.path);
  }
  if (!Array.isArray(packet.artifactInventory))
    throw new Error(
      "bundled run packet must contain a digest-bound artifactInventory",
    );
  ...
  const collectBoundPaths = (value) => {
    if (typeof value === "string") add(value);
      ...
      for (const entry of value) collectBoundPaths(entry);
    else if (value && typeof value === "object")
      for (const entry of Object.values(value)) collectBoundPaths(entry);
  };
  while (pending.length > 0) {
    ...
    try {
      document = JSON.parse(
        readFileSync(path.join(bundleRoot, ...relativePath.split("/")), "utf8"),
      );
    } catch {
      continue;
    }
  ...
  if (JSON.stringify(expected) !== JSON.stringify(actual)) {
    throw new Error(
      "artifact bundle inventory must equal the transitive digest-bound run-packet closure",
    );
  }
}
...
function withValidationWorktree(repoRoot, head, callback) {
  const temporaryRoot = realpathSync(
    mkdtempSync(path.join(os.tmpdir(), "valdris-run-acceptance-")),
  );
...
async function main() {
  ...
  if (args.help) {
    console.log(
      `Usage: node scripts/run-acceptance.mjs --repo . [--bundle <extracted-artifact-directory>] [--source-commit <full-git-sha>]\n\nVALDRIS_ARTIFACT_BUNDLE and VALDRIS_SOURCE_COMMIT provide injection-safe defaults for CI. The bundle must contain ${MANIFEST_NAME} using ${BUNDLE_SCHEMA}.`,
    );
    return;
  }
  if (!args.bundle || !args.sourceCommit)
    throw new Error("--bundle and --source-commit are required");
  if (!GIT_OBJECT_ID.test(args.sourceCommit))
    throw new Error("--source-commit must be a lowercase full Git object ID");
  const trustPin = process.env.UASH_REVIEW_TRUST_SHA256 || "";
  if (!SHA256.test(trustPin))
    throw new Error(
      "operator-held UASH_REVIEW_TRUST_SHA256 must be a lowercase SHA-256 digest",
    );

  const repoInput = path.resolve(args.repo);
  if (!existsSync(repoInput) || !lstatSync(repoInput).isDirectory())
    throw new Error("--repo must identify an existing directory");
  const repoRoot = realpathSync(repoInput);
  const expectedScriptDir = path.join(repoRoot, ".valdris-harness", "scripts");
  if (
    !existsSync(expectedScriptDir) ||
    realpathSync(expectedScriptDir) !== realpathSync(SCRIPT_DIR)
  ) {
    throw new Error(
      "run acceptance must execute from the source checkout's committed .valdris-harness runtime",
    );
  }
  const head = git(repoRoot, ["rev-parse", "HEAD"]).trim().toLowerCase();
  if (head !== args.sourceCommit)
    throw new Error("source checkout HEAD does not match --source-commit");
  assertCleanSourceCheckout(repoRoot);

  const bundleInput = path.resolve(args.bundle);
  if (
    !existsSync(bundleInput) ||
    lstatSync(bundleInput).isSymbolicLink() ||
    !lstatSync(bundleInput).isDirectory()
  )
    throw new Error("--bundle must identify a real non-symlink directory");
  const bundleRoot = realpathSync(bundleInput);
  if (isWithin(repoRoot, bundleRoot))
    throw new Error("artifact bundle must be outside the source checkout");
  const manifest = readAndValidateManifest(bundleRoot, args.sourceCommit);
  const packetEntry = manifest.files.find(
    (file) => file.path === "run/packet.json",
  );
  let packet;
  try {
    packet = JSON.parse(
      readFileSync(
        path.join(bundleRoot, ...packetEntry.path.split("/")),
        "utf8",
      ),
    );
  } catch {
    throw new Error("bundled run/packet.json must be valid JSON");
  }
  if (packet.commit !== head)
    throw new Error(
      "bundled run packet commit does not match the exact source checkout HEAD",
    );
  for (const file of manifest.files) assertSafeDestination(repoRoot, file.path);
  packetClosurePaths(bundleRoot, manifest, packet);
  const bundlePrivacy = validatePrivacy(bundleRoot, {
    includes: manifest.files.map((file) => file.path),
  });
  if (!bundlePrivacy.ok)
    throw new Error(
      `artifact bundle privacy validation failed: ${bundlePrivacy.findings.map((finding) => `${finding.category}:${finding.fingerprint}`).join(", ")}`,
    );
  const env = childEnvironment(trustPin);
  let gates = [];
  const targetGitPrefix = git(repoRoot, ["rev-parse", "--show-prefix"])
    .trim()
    .replaceAll("\\", "/")
    .replace(/^\.\//, "")
    .replace(/\/+$/, "");
  if (
    targetGitPrefix === ".." ||
    targetGitPrefix.startsWith("../") ||
    path.posix.isAbsolute(targetGitPrefix)
  ) {
  ...
  withValidationWorktree(repoRoot, head, (validationWorktreeRoot) => {
    const validationRoot = path.join(
      validationWorktreeRoot,
      ...targetGitPrefix.split("/").filter(Boolean),
    );
    hydrateArtifacts(validationRoot, bundleRoot, manifest.files);
    const privacy = validatePrivacy(validationRoot, {
      includes: manifest.files.map((file) => file.path),
    });
    if (!privacy.ok)
      throw new Error(
        `staged artifact privacy validation failed: ${privacy.findings.map((finding) => `${finding.category}:${finding.fingerprint}`).join(", ")}`,
      );
    ...
    gates = [
      ...
      [
        "enterprise and AI assurance",
        "enterprise-ai-gate-all.mjs",
        ["--repo", validationRoot],
      ],
    ];
    if (manifest.files.some((file) => file.path === "rca/rca.json"))
      gates.push(["RCA", "rca-gate.mjs", ["--repo", validationRoot]]);
    gates.push(
  ...
  const removeInterruptionHandlers =
    installInterruptionRollback(finalTransaction);
  try {
    const reboundHead = git(repoRoot, ["rev-parse", "HEAD"])
      .trim()
      .toLowerCase();
    if (reboundHead !== head)
      throw new Error("source checkout HEAD changed before artifact commit");
    assertCleanSourceCheckout(repoRoot);
    ...
    JSON.stringify(
      {
        ...
        pathEnvironmentKeys: Object.keys(env).filter(

...
function card(x, y, w, h, title, opts = {}) {
  ...
  const subLines = sub
    ? Array.isArray(sub)
      ? sub
      : wrap(sub, Math.max(22, Math.floor(w / 7.4)))
    : [];
...
function repoOperatingMap() {
  let s = shell(
    "Valdris SDLC Harness — Whole Repo Operating Map",
    "Grouped by responsibility: front doors, docs, generator, runtime, repo intelligence, gates, and evidence.",
  );
  ...
  [
    ...
    "ENTERPRISE_PROOF_BANK.md",
    "OPERATING_INTELLIGENCE_LAYER.md",
    "TEST_DAY_ACCEPTANCE_GATES.md",
    "HARNESS_REPO_MAP.md",
  ].forEach((t, i) => {
  ...
  s += miniCard(444, 216, 270, 46, "commission-harness.mjs", colors.amber, {
    size: 15,
  });
  s += miniCard(444, 278, 270, 46, "templates/", colors.amber, { size: 16 });
  s += miniCard(444, 340, 270, 46, "generated harness pack", colors.green, {
    size: 15,
  });
  ...
  [
    ...
    s += miniCard(
      ...
      35,
      t,
      i === 0 ? colors.rose : colors.green,
      { size: 13 },
    );
  });

  s += cluster(1200, 128, 340, 278, "Visual control-plane app", colors.cyan);
  ["app/", "components/", "lib/", "app/api/runs/demo/events"].forEach(
    (t, i) => {
  ...
  ["runs/_run-template/", "session/events.jsonl", "handoff/final.md"].forEach(
    (t, i) => {
  ...
  s += label(
    66,
    824,
    "Read this as an operating system map, not a raw import graph. The repo is organized around agent front doors, runtime events, proof gates, and handoff evidence.",
...
function flowMonitor() {
  let s = shell(
    "Valdris SDLC Harness — Visual Flow Monitor",
    "Blueprint / Live Run / Replay modes keep static topology separate from real telemetry.",
  );
  s += `<rect x="64" y="122" width="1472" height="704" rx="28" fill="#030712" opacity="0.86" stroke="#334155" stroke-width="1.4" filter="url(#shadow)"/>`;
  s += label(96, 172, "Run board", {
    size: 23,
    color: colors.text,
    weight: 850,
  });
  ...
  s += label(1276, 172, "demo topology only", {
    size: 14,
    color: colors.amber,
    weight: 800,
  });
  ...
  s += label(canvasX + 24, canvasY + 36, "N8N-style SDLC swimlane", {
    size: 17,
    color: colors.muted,
    weight: 800,
  });
  const nodes = [
    ["intake", colors.green, 0],
    ["route", colors.green, 1],
    ["GitNexus", colors.green, 2],
    ["design-anchors", colors.green, 3],
    ["system-design", colors.indigo, 4],
    ["production-readiness", colors.amber, 5],
    ["cloud-platform", colors.amber, 6],
    ["implement", colors.cyan, 7],
    ["redzone", colors.rose, 8],
    ["qa-break-it", colors.violet, 9],
    ["prove", colors.rose, 10],
    ["live-smoke", colors.teal, 11],
    ["self-heal", colors.amber, 12],
    ["handoff", colors.green, 13],
  ];
  ...
  nodes.forEach(([name, color], i) => {
    ...
    const y = ny0 + row * 122;
    s += miniCard(x, y, nw, nh, name, color, {
      size: name.length > 12 ? 10.5 : name.length > 10 ? 11.5 : 13,
    });
    if (i !== 6 && i !== 13) {
  ...
  s += label(1154, 250, "Selected node", {
    size: 15,
    color: colors.muted,
    weight: 800,
  });
  ...
  s += card(1154, 318, 318, 86, "Required artifact", {
    sub: "proof/proof.json must exist and pass",
    stroke: colors.rose,
    titleSize: 18,
    subSize: 13,
  });
  s += card(1154, 424, 318, 76, "Last event", {
    sub: "node.entered from connector bridge",
    stroke: colors.cyan,
    titleSize: 18,
    subSize: 13,
  });
  s += card(1154, 520, 318, 62, "Gate result", {
    sub: "finish-line not satisfied",
    stroke: colors.amber,
    titleSize: 18,
    subSize: 13,
  });
  ...
  s += label(120, ledgerY + 36, "Event stream + artifact ledger", {
    size: 18,
    color: colors.text,
    weight: 850,
  });
  [
    ...
    s += label(x + 110, ledgerY + 88, row[1], {
      size: row[1].length > 12 ? 12 : 14,
      color: row[3],
      weight: 850,
    });
    s += label(x + 14, ledgerY + 126, row[2], {
      size: 13,
      color: colors.muted,
      weight: 700,
    });
...
function productionReadiness() {
  let s = shell(
    "Valdris SDLC Harness — 13-Layer Production Readiness Pack",
    "A production-impacting run cannot be called done until touched layers are assessed with proof.",
  );
  s += card(560, 126, 480, 74, "production-impacting run", {
    sub: "feature, deploy, data, auth, cloud, incident, or reliability work",
    stroke: colors.cyan,
    align: "center",
    fill: "url(#cyanFill)",
  });
  s += lineArrow(800, 204, 800, 244, colors.cyan);
  s += card(540, 246, 520, 76, "production readiness layer pack", {
    sub: "mark each layer required / passed / failed / pending / skipped with reason",
    stroke: colors.amber,
    align: "center",
    fill: "url(#amberFill)",
  });
  const groups = [
    ...
    [
      ...
      382,
      colors.teal,
      ["12 Logs / observability", "13 Availability / recovery / DR"],
    ],
  ];
  for (const [title, x, y, color, items] of groups) {
    ...
    s += lineArrow(
      800,
      324,
      x + (title === "Platform surface" ? 250 : 195),
      y - 14,
      color,
      title === "Platform surface" ? "arrowAmber" : "arrow",
      1.6,
    );
  }
  s += card(
    ...
    740,
    66,
    "production/layer-assessment.json → finish-line gate",
    {
      sub: "status must be required/passed/failed/pending/skipped with reason",
      stroke: colors.green,
      titleSize: 21,
      subSize: 13,
      align: "center",
  ...
  steps.forEach(([t, color], i) => {
    ...
    if (i < steps.length - 1)
      s += lineArrow(
        ...
        296,
        color,
        color === colors.amber ? "arrowAmber" : "arrow",
      );
  });
...
function universalAdapter() {
  let s = shell(
    "Valdris SDLC Harness — Universal Core vs Project Adapter",
    "The product is universal; every repo gets a generated operating layer.",
  );
  s += card(80, 180, 230, 78, "team or operator", {
    stroke: colors.cyan,
    align: "center",
  });
  s += card(80, 312, 230, 78, "target repository", {
    stroke: colors.blue,
    align: "center",
  });
  s += card(380, 160, 260, 92, "commissioning interview", {
    sub: "human answers + repo rules",
    stroke: colors.cyan,
    align: "center",
  });
  s += card(380, 300, 260, 92, "GitNexus index", {
    sub: "code intelligence + anchors",
    stroke: colors.blue,
    align: "center",
  });
  ...
  s += label(740, 170, "Universal core", {
    size: 24,
    color: colors.indigo,
    weight: 900,
  });
  ...
  [
    ...
    "Red Zone owners",
    "enabled lanes",
    "answer + handoff style",
  ].forEach(
    (t, i) =>
      (s += miniCard(1142, 205 + i * 54, 266, 36, t, colors.amber, {
        size: 13,
      })),
  );
  ...
  s += card(392, 624, 280, 102, "Project-specific harness pack", {
    sub: "AGENTS.md / CLAUDE.md / slash command / Codex prompt / run packet template",
    stroke: colors.green,
    titleSize: 19,
    subSize: 12,
    maxChars: 28,
  ...
  s += card(1048, 624, 280, 102, "proof-backed handoff", {
    sub: "answers cite artifacts and gate results",
    stroke: colors.green,
    titleSize: 19,
    subSize: 12,
    maxChars: 28,
...
function generatedPack() {
  let s = shell(
    "Valdris SDLC Harness — Generated Harness Pack",
    "Commissioning turns a normal repo into an AI-operable repo with front doors, rules, run packets, and helper scripts.",
  );
  const inputs = [
    ["target repo", colors.blue],
    ["human answers", colors.cyan],
    ["GitNexus index", colors.indigo],
  ];
  inputs.forEach(([t, color], i) => {
    const x = 220 + i * 240;
    s += card(x, 150, 190, 72, t, {
      stroke: color,
      align: "center",
      titleSize: 19,
    });
    s += arrow(
      ...
      310,
      color,
      color === colors.indigo ? "arrowAmber" : "arrow",
      1.5,
    );
  });
  s += card(620, 300, 360, 92, "commission-harness.mjs", {
    sub: "repo-specific operating kit generator",
    stroke: colors.amber,
    align: "center",
    fill: "url(#amberFill)",
    titleSize: 24,
  ...
  const groups = [
    ...
    [
      ...
      488,
      colors.amber,
      ["project-adapter.json", "project.yaml", "00_MAP.md", "CONTEXT.md"],
    ],
    [
      "Operating docs",
      698,
      ...
      [
        ...
        "Red Zone Rules.md",
        "Production Readiness Layers.md",
        "QA and Live Smoke.md",
      ],
    ],
    [
      "Run packet",
      ...
      488,
      colors.green,
      ["runs/_run-template/", "commissioning-review.md"],
    ],
    [
      "Helper scripts",
      1268,
      488,
      colors.teal,
      ["uash-emit-event.mjs", "code-intelligence-scan.mjs"],
    ],
  ];
  groups.forEach(([title, x, y, color, items]) => {
    ...
    items.forEach(
      (t, i) =>
        (s += miniCard(x + 18, y + 66 + i * 42, w - 36, 32, t, color, {
          size: t.length > 22 ? 11 : 13,
        })),
    );
    s += arrow(
      800,
  ...
  s += label(
    80,
    820,
    "Output shape: generated instructions + repo adapter + validation docs + evidence packet + connector helpers.",
    { size: 17, color: colors.muted, weight: 750 },
  );
  return s + endSvg();
}

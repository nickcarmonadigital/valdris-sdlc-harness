...
function main() {
  ...
  try {
    ...
    writeFileSync(path.join(root, "docs", "incident-process.md"), "broken\n", "utf8");
    ...
    const reviewTrustSha256 = digest(canonicalJson(readJson(path.join(root, ".valdris-harness", "controls", "review-trust.v1.json"))));
    ...
    runGit(root, ["add", ".gitignore", "fixture-status.txt", "cloud/service-map.json", "docs/incident-process.md", "packages/nested-app/README.md", ".valdris-harness"]);
    ...
    const documentationRedOutput = path.join(root, "proof", "documentation-red.json");
    const documentationRed = run("proof-runner.mjs", [
      "--repo", root,
      "--run-id", "EXAMPLE-RUN-001",
      "--commit", PRE_FIX_COMMIT,
      "--environment", "synthetic-test",
      "--output", documentationRedOutput,
      "--repeat", "2",
      "--causal-input", "docs/incident-process.md",
      "--red-baseline",
      "--",
      ...DOCUMENTATION_REGRESSION_COMMAND,
    ], { cwd: root });
    assert(documentationRed.status === 0, `documentation-process red baseline was not accepted:\n${documentationRed.stdout}\n${documentationRed.stderr}`);
    const documentationRedArtifact = readJson(documentationRedOutput);
    assert(documentationRedArtifact.outcome?.status === "red-confirmed", "documentation-process red baseline did not persist red-confirmed");
    assert(documentationRedArtifact.causalInputs?.[0]?.path === "docs/incident-process.md", "documentation-process red proof did not bind its canonical causal input path");
    assert(documentationRedArtifact.causalInputs?.[0]?.beforeSha256 === digest("broken\n"), "documentation-process red proof did not bind the exact pre-command causal input bytes");
    assert(documentationRedArtifact.causalInputs?.[0]?.afterSha256 === digest("broken\n"), "documentation-process red proof did not bind the exact post-command causal input bytes");
    assert(documentationRedArtifact.bindings?.causalInputsSha256 === digest(canonicalJson(documentationRedArtifact.causalInputs)), "documentation-process red proof did not envelope-bind its causal inputs");

    const documentationCoverRedOutput = path.join(root, "proof", "documentation-cover-red.json");
    const documentationCoverRed = run("proof-runner.mjs", [
      "--repo", root, "--run-id", "EXAMPLE-RUN-001", "--commit", PRE_FIX_COMMIT,
      "--environment", "synthetic-test", "--output", documentationCoverRedOutput,
      "--causal-input", "fixture-status.txt", "--red-baseline", "--",
      ...DOCUMENTATION_REGRESSION_COMMAND,
    ], { cwd: root });
    assert(documentationCoverRed.status === 0, `documentation cover-change red proof failed:\n${documentationCoverRed.stdout}\n${documentationCoverRed.stderr}`);
    const documentationCoverRedArtifact = readJson(documentationCoverRedOutput);

    const nestedDocumentationRedOutput = path.join(nestedTarget, "proof", "nested-documentation-red.json");
    const nestedDocumentationRed = run("proof-runner.mjs", [
      "--repo", nestedTarget, "--run-id", "EXAMPLE-NESTED-RUN-001", "--commit", PRE_FIX_COMMIT,
      "--environment", "synthetic-test", "--output", "proof/nested-documentation-red.json",
      "--causal-input", "README.md", "--red-baseline", "--",
      ...NESTED_DOCUMENTATION_REGRESSION_COMMAND,
    ], { cwd: nestedTarget });
    assert(nestedDocumentationRed.status === 0, `nested documentation red proof failed:\n${nestedDocumentationRed.stdout}\n${nestedDocumentationRed.stderr}`);
    const nestedDocumentationRedArtifact = readJson(nestedDocumentationRedOutput);
    assert(nestedDocumentationRedArtifact.source?.targetPath === "packages/nested-app", "nested documentation proof did not bind its Git target prefix");
    assert(nestedDocumentationRedArtifact.causalInputs?.[0]?.path === "README.md", "nested documentation proof did not keep its causal path target-relative");

    const opaqueEnvironmentRedOutput = path.join(root, "proof", "opaque-environment-red.json");
    const opaqueEnvironmentRedValue = "opaque-feature-red-271828";
    const opaqueEnvironmentRed = run("proof-runner.mjs", [
      "--repo", root, "--run-id", "EXAMPLE-RUN-001", "--commit", PRE_FIX_COMMIT,
      "--environment", "synthetic-test", "--output", opaqueEnvironmentRedOutput,
      "--pass-env", "FEATURE_FLAG", "--red-baseline", "--", ...OPAQUE_ENV_REGRESSION_COMMAND,
    ], { cwd: root, env: { ...process.env, FEATURE_FLAG: opaqueEnvironmentRedValue } });
    assert(opaqueEnvironmentRed.status === 0, `opaque environment red proof failed:\n${opaqueEnvironmentRed.stdout}\n${opaqueEnvironmentRed.stderr}`);
    ...
    const allowlistedEnvironmentRedOutput = path.join(root, "proof", "allowlisted-environment-red.json");
    const allowlistedEnvironmentRed = run("proof-runner.mjs", [
      "--repo", root, "--run-id", "EXAMPLE-RUN-001", "--commit", PRE_FIX_COMMIT,
      "--environment", "synthetic-test", "--output", allowlistedEnvironmentRedOutput,
      "--red-baseline", "--", ...ALLOWLISTED_ENV_REGRESSION_COMMAND,
    ], { cwd: root, env: { ...process.env, CI: allowlistedEnvironmentRedValue } });
    assert(allowlistedEnvironmentRed.status === 0, `allowlisted environment red proof failed:\n${allowlistedEnvironmentRed.stdout}\n${allowlistedEnvironmentRed.stderr}`);
    const allowlistedEnvironmentRedArtifact = readJson(allowlistedEnvironmentRedOutput);

    const opaqueSecretRedValue = "opaque-credential-red-161803";
    const opaqueSecretRedOutput = path.join(root, "proof", "opaque-secret-red.json");
    const opaqueSecretRed = run("proof-runner.mjs", [
      "--repo", root, "--run-id", "EXAMPLE-RUN-001", "--commit", PRE_FIX_COMMIT,
      "--environment", "synthetic-test", "--output", opaqueSecretRedOutput, "--red-baseline", "--",
      ...OPAQUE_SECRET_ARGV_REGRESSION_COMMAND(opaqueSecretRedValue),
    ], { cwd: root });
    assert(opaqueSecretRed.status === 0, `opaque secret argv red proof failed:\n${opaqueSecretRed.stdout}\n${opaqueSecretRed.stderr}`);
    ...
    writeFileSync(path.join(root, "docs", "incident-process.md"), "fixed\n", "utf8");
    writeFileSync(path.join(nestedTarget, "README.md"), "fixed\n", "utf8");
    runGit(root, ["add", "fixture-status.txt", "cloud/service-map.json", "docs/incident-process.md", "packages/nested-app/README.md"]);
    ...
    const documentationGreenOutput = path.join(root, "proof", "documentation-green.json");
    const documentationGreen = run("proof-runner.mjs", [
      "--repo", root,
      "--run-id", "EXAMPLE-RUN-001",
      "--commit", POST_FIX_COMMIT,
      "--environment", "synthetic-test",
      "--output", documentationGreenOutput,
      "--repeat", "2",
      "--causal-input", "docs/incident-process.md",
      "--",
      ...DOCUMENTATION_REGRESSION_COMMAND,
    ], { cwd: root });
    assert(documentationGreen.status === 0, `documentation-process green proof failed:\n${documentationGreen.stdout}\n${documentationGreen.stderr}`);
    const documentationGreenArtifact = readJson(documentationGreenOutput);
    assert(documentationGreenArtifact.outcome?.status === "passed", "documentation-process green proof did not persist passed");
    assert(documentationGreenArtifact.causalInputs?.[0]?.beforeSha256 === digest("fixed\n"), "documentation-process green proof did not bind the exact pre-command causal input bytes");
    assert(documentationGreenArtifact.causalInputs?.[0]?.afterSha256 === digest("fixed\n"), "documentation-process green proof did not bind the exact post-command causal input bytes");
    assert(documentationGreenArtifact.bindings?.causalInputsSha256 === digest(canonicalJson(documentationGreenArtifact.causalInputs)), "documentation-process green proof did not envelope-bind its causal inputs");

    const documentationCoverGreenOutput = path.join(root, "proof", "documentation-cover-green.json");
    const documentationCoverGreen = run("proof-runner.mjs", [
      "--repo", root, "--run-id", "EXAMPLE-RUN-001", "--commit", POST_FIX_COMMIT,
      "--environment", "synthetic-test", "--output", documentationCoverGreenOutput,
      "--causal-input", "fixture-status.txt", "--",
      ...DOCUMENTATION_REGRESSION_COMMAND,
    ], { cwd: root });
    assert(documentationCoverGreen.status === 0, `documentation cover-change green proof failed:\n${documentationCoverGreen.stdout}\n${documentationCoverGreen.stderr}`);
    const documentationCoverGreenArtifact = readJson(documentationCoverGreenOutput);

    const nestedDocumentationGreenOutput = path.join(nestedTarget, "proof", "nested-documentation-green.json");
    const nestedDocumentationGreen = run("proof-runner.mjs", [
      "--repo", nestedTarget, "--run-id", "EXAMPLE-NESTED-RUN-001", "--commit", POST_FIX_COMMIT,
      "--environment", "synthetic-test", "--output", "proof/nested-documentation-green.json",
      "--causal-input", "README.md", "--", ...NESTED_DOCUMENTATION_REGRESSION_COMMAND,
    ], { cwd: nestedTarget });
    assert(nestedDocumentationGreen.status === 0, `nested documentation green proof failed:\n${nestedDocumentationGreen.stdout}\n${nestedDocumentationGreen.stderr}`);
    const nestedDocumentationGreenArtifact = readJson(nestedDocumentationGreenOutput);

    const opaqueEnvironmentGreenOutput = path.join(root, "proof", "opaque-environment-green.json");
    const opaqueEnvironmentGreenValue = "opaque-feature-green-314159";
    const opaqueEnvironmentGreen = run("proof-runner.mjs", [
      "--repo", root, "--run-id", "EXAMPLE-RUN-001", "--commit", POST_FIX_COMMIT,
      "--environment", "synthetic-test", "--output", opaqueEnvironmentGreenOutput,
      "--pass-env", "FEATURE_FLAG", "--", ...OPAQUE_ENV_REGRESSION_COMMAND,
    ], { cwd: root, env: { ...process.env, FEATURE_FLAG: opaqueEnvironmentGreenValue } });
    assert(opaqueEnvironmentGreen.status === 0, `opaque environment green proof failed:\n${opaqueEnvironmentGreen.stdout}\n${opaqueEnvironmentGreen.stderr}`);
    const opaqueEnvironmentGreenArtifact = readJson(opaqueEnvironmentGreenOutput);
    for (const [label, opaqueArtifact, rawValue] of [
      ...
      assert(opaqueArtifact.command?.executionInputs?.stability === "opaque-dynamic", `${label} pass-env proof was not marked opaque-dynamic`);
      assert(opaqueArtifact.command?.executionInputs?.causalIdentityEligible === false, `${label} pass-env proof remained RCA-eligible`);
      assert(!JSON.stringify(opaqueArtifact).includes(rawValue), `${label} pass-env proof persisted the raw effective value`);
    }

    const allowlistedEnvironmentGreenValue = "allowlisted-environment-green-314159";
    const allowlistedEnvironmentGreenOutput = path.join(root, "proof", "allowlisted-environment-green.json");
    const allowlistedEnvironmentGreen = run("proof-runner.mjs", [
      "--repo", root, "--run-id", "EXAMPLE-RUN-001", "--commit", POST_FIX_COMMIT,
      "--environment", "synthetic-test", "--output", allowlistedEnvironmentGreenOutput,
      "--", ...ALLOWLISTED_ENV_REGRESSION_COMMAND,
    ], { cwd: root, env: { ...process.env, CI: allowlistedEnvironmentGreenValue } });
    assert(allowlistedEnvironmentGreen.status === 0, `allowlisted environment green proof failed:\n${allowlistedEnvironmentGreen.stdout}\n${allowlistedEnvironmentGreen.stderr}`);
    const allowlistedEnvironmentGreenArtifact = readJson(allowlistedEnvironmentGreenOutput);
    assert(allowlistedEnvironmentRedArtifact.command?.executionInputs?.stability === "static", "allowlisted red proof was not kept RCA-eligible");
    assert(allowlistedEnvironmentGreenArtifact.command?.executionInputs?.stability === "static", "allowlisted green proof was not kept RCA-eligible");
    assert(allowlistedEnvironmentRedArtifact.command?.executionInputs?.allowlistedEnvironmentSha256 !== allowlistedEnvironmentGreenArtifact.command?.executionInputs?.allowlistedEnvironmentSha256, "allowlisted environment changes did not change the persisted input identity");
    assert(allowlistedEnvironmentRedArtifact.bindings?.commandSha256 !== allowlistedEnvironmentGreenArtifact.bindings?.commandSha256, "allowlisted environment changes did not change the bound command identity");
    assert(!JSON.stringify(allowlistedEnvironmentRedArtifact).includes(allowlistedEnvironmentRedValue) && !JSON.stringify(allowlistedEnvironmentGreenArtifact).includes(allowlistedEnvironmentGreenValue), "allowlisted environment identity persisted raw environment values");

    const opaqueSecretGreenValue = "opaque-credential-green-141421";
    const opaqueSecretGreenOutput = path.join(root, "proof", "opaque-secret-green.json");
    const opaqueSecretGreen = run("proof-runner.mjs", [
      "--repo", root, "--run-id", "EXAMPLE-RUN-001", "--commit", POST_FIX_COMMIT,
      "--environment", "synthetic-test", "--output", opaqueSecretGreenOutput, "--",
      ...OPAQUE_SECRET_ARGV_REGRESSION_COMMAND(opaqueSecretGreenValue),
    ], { cwd: root });
    assert(opaqueSecretGreen.status === 0, `opaque secret argv green proof failed:\n${opaqueSecretGreen.stdout}\n${opaqueSecretGreen.stderr}`);
    ...
    for (const [label, opaqueArtifact, rawValue] of [
      ...
      assert(opaqueArtifact.command?.executionInputs?.stability === "opaque-dynamic", `${label} secret-argv proof was not marked opaque-dynamic`);
      assert(opaqueArtifact.command?.executionInputs?.causalIdentityEligible === false, `${label} secret-argv proof remained RCA-eligible`);
      assert(!JSON.stringify(opaqueArtifact).includes(rawValue), `${label} secret-argv proof persisted the raw value`);
    }
    ...
    const databaseSecret = ["postgres", "://fixture-user:", "fixture-password", "@example.invalid/db"].join("");
    const dsnSecret = "Server=example.invalid;Password=fixture-dsn-secret";
    const connectionSecret = ["mongodb", "://fixture-user:", "fixture-password", "@example.invalid/data"].join("");
    ...
    const documentationCausalEvidencePath = path.join(root, "rca", "documentation-causal-evidence.json");
    const documentationFixEvidencePath = path.join(root, "rca", "documentation-fix-evidence.json");
    writeJson(documentationCausalEvidencePath, { schema: "synthetic.documentation-causal-evidence.v1", observation: "The regression command reads the broken incident process document directly." });
    writeJson(documentationFixEvidencePath, { schema: "synthetic.documentation-change-evidence.v1", change: "The causal incident process document now contains the passing state." });
    const documentationRca = structuredClone(rcaDocument);
    documentationRca.symptom = { id: "EXAMPLE-SYMPTOM-001", summary: "The incident process document leaves the regression command in a failing state.", evidenceIds: ["EXAMPLE-REPRODUCTION-001"] };
    documentationRca.rootCause = {
      summary: "The incident process document encodes the broken behavior.",
      causalMechanism: "The regression command reads the document as its direct runtime input.",
    ...
    documentationRca.regression = {
      ...
      summary: "The same document-backed command fails before the process repair and passes after it.",
    ...
    Object.assign(documentationRca.evidence[0], {
      ...
      observation: "The document-backed symptom is reproduced by the exact bounded regression command.",
    });
    Object.assign(documentationRca.evidence[1], {
      ...
      observation: "The diagnosis binds the broken process document to the reproduced command input.",
    });
    Object.assign(documentationRca.evidence[2], {
      ...
      observation: "The change evidence identifies the corrected causal process document.",
    });
    Object.assign(documentationRca.evidence[3], {
      ...
      regressionId: documentationRca.regression.id,
      observation: "The same document-backed regression command passes after the process repair.",
    });
    writeJson(path.join(root, "rca", "documentation-process.json"), documentationRca);
    const documentationRcaResult = run("rca-gate.mjs", ["--repo", root, "--file", "rca/documentation-process.json"], { cwd: root });
    assert(documentationRcaResult.status === 0, `typed documentation-process RCA was rejected:\n${documentationRcaResult.stdout}\n${documentationRcaResult.stderr}`);

    const nestedCausalEvidencePath = path.join(nestedTarget, "rca", "nested-documentation-causal-evidence.json");
    const nestedFixEvidencePath = path.join(nestedTarget, "rca", "nested-documentation-fix-evidence.json");
    writeJson(nestedCausalEvidencePath, { schema: "synthetic.documentation-causal-evidence.v1", observation: "The nested regression reads the target-relative README directly." });
    writeJson(nestedFixEvidencePath, { schema: "synthetic.documentation-change-evidence.v1", change: "The nested target README now contains the passing process state." });
    const nestedDocumentationRca = structuredClone(documentationRca);
    nestedDocumentationRca.runId = "EXAMPLE-NESTED-RUN-001";
    nestedDocumentationRca.symptom.summary = "The nested target README leaves its regression command in a failing state.";
    nestedDocumentationRca.rootCause = {
      summary: "The nested target README encodes the broken process behavior.",
      causalMechanism: "The target-relative regression command reads README.md directly.",
      causalClass: "documentation-process",
    ...
    nestedDocumentationRca.regression = {
      ...
      summary: "The same target-relative README regression fails before and passes after the nested repair.",
      commandSha256: nestedDocumentationGreenArtifact.bindings.commandSha256,
      failureSignature: "nested-documentation-regression",
      causalInputs: [{ path: "README.md", preFixSha256: digest("broken\n"), postFixSha256: digest("fixed\n") }],
    };
    Object.assign(nestedDocumentationRca.evidence[0], {
      ...
      proofBindingSha256: nestedDocumentationRedArtifact.bindings.envelopeSha256,
    ...
    Object.assign(nestedDocumentationRca.evidence[3], {
      ...
      proofBindingSha256: nestedDocumentationGreenArtifact.bindings.envelopeSha256,
      regressionId: nestedDocumentationRca.regression.id,
    });
    writeJson(path.join(nestedTarget, "rca", "nested-documentation-process.json"), nestedDocumentationRca);
    const nestedDocumentationRcaResult = run("rca-gate.mjs", ["--repo", nestedTarget, "--file", "rca/nested-documentation-process.json"], { cwd: nestedTarget });
    assert(nestedDocumentationRcaResult.status === 0, `nested target documentation-process RCA was rejected:\n${nestedDocumentationRcaResult.stdout}\n${nestedDocumentationRcaResult.stderr}`);
    ...
    Object.assign(opaqueEnvironmentRca.regression, {
      ...
      summary: "Opaque passed environment values must not masquerade as one causal command identity.",
    ...
    Object.assign(opaqueEnvironmentRca.evidence[3], {
      ...
      proofBindingSha256: opaqueEnvironmentGreenArtifact.bindings.envelopeSha256,
      regressionId: opaqueEnvironmentRca.regression.id,
    });
    writeJson(path.join(root, "rca", "opaque-environment-identity.json"), opaqueEnvironmentRca);
    expectFailure(
      run("rca-gate.mjs", ["--repo", root, "--file", "rca/opaque-environment-identity.json"], { cwd: root }),
    ...
    Object.assign(allowlistedEnvironmentRca.regression, {
      ...
      summary: "Distinct allowlisted environment values must produce distinct causal command identities.",
    ...
    Object.assign(allowlistedEnvironmentRca.evidence[0], {
      ...
      proofBindingSha256: allowlistedEnvironmentRedArtifact.bindings.envelopeSha256,
    ...
    Object.assign(allowlistedEnvironmentRca.evidence[3], {
      ...
      proofBindingSha256: allowlistedEnvironmentGreenArtifact.bindings.envelopeSha256,
      regressionId: allowlistedEnvironmentRca.regression.id,
    });
    writeJson(path.join(root, "rca", "allowlisted-environment-identity.json"), allowlistedEnvironmentRca);
    expectFailure(
      run("rca-gate.mjs", ["--repo", root, "--file", "rca/allowlisted-environment-identity.json"], { cwd: root }),
    ...
    Object.assign(opaqueSecretArgvRca.regression, {
      ...
      summary: "Distinct opaque secret argv values must not masquerade as one causal command identity.",
    ...
    writeJson(path.join(root, "rca", "opaque-secret-argv-identity.json"), opaqueSecretArgvRca);
    expectFailure(
      run("rca-gate.mjs", ["--repo", root, "--file", "rca/opaque-secret-argv-identity.json"], { cwd: root }),
    ...
    writeJson(path.join(root, "rca", "documentation-process-missing-causal-input.json"), missingDocumentationCausalInput);
    expectFailure(
      run("rca-gate.mjs", ["--repo", root, "--file", "rca/documentation-process-missing-causal-input.json"], { cwd: root }),
    ...
    const mismatchedDocumentationCausalInput = structuredClone(documentationRca);
    mismatchedDocumentationCausalInput.regression.causalInputs[0].preFixSha256 = "0".repeat(64);
    writeJson(path.join(root, "rca", "documentation-process-mismatched-causal-input.json"), mismatchedDocumentationCausalInput);
    expectFailure(
      run("rca-gate.mjs", ["--repo", root, "--file", "rca/documentation-process-mismatched-causal-input.json"], { cwd: root }),
    ...
    Object.assign(documentationCoverChangeRca.evidence[3], {
      ...
      proofBindingSha256: documentationCoverGreenArtifact.bindings.envelopeSha256,
    });
    writeJson(path.join(root, "rca", "documentation-process-cover-change.json"), documentationCoverChangeRca);
    expectFailure(
      run("rca-gate.mjs", ["--repo", root, "--file", "rca/documentation-process-cover-change.json"], { cwd: root }),
    ...
    writeJson(path.join(root, "rca", "documentation-process-untyped.json"), untypedDocumentationRca);
    expectFailure(
      run("rca-gate.mjs", ["--repo", root, "--file", "rca/documentation-process-untyped.json"], { cwd: root }),
    ...
    writeJson(path.join(root, "rca", "documentation-process-mismatched.json"), mismatchedDocumentationRca);
    expectFailure(
      run("rca-gate.mjs", ["--repo", root, "--file", "rca/documentation-process-mismatched.json"], { cwd: root }),
    ...
    writeJson(routePath, { schema: "uash.route.v2", status: "synthetic-scout-evidence" });
    ...
    const reviewDocument = {
      ...
      roleProvenance: {
        ...
        scout: {
          ...
          evidence: { kind: "artifact", path: "run/route.json", sha256: routeSha256 },
        },
        implementer: {
          ...
          evidence: { kind: "artifact", path: "proof/portable.json", sha256: proofSha256 },
        },
        verifier: {
    ...
    const malformedPortablePath = path.join(root, "proof", "malformed-portable.json");
    ...
    malformedPortableReview.subject = { artifact: "proof/malformed-portable.json", sha256: malformedPortableSha256 };
    malformedPortableReview.roleProvenance.implementer.evidence = { kind: "artifact", path: "proof/malformed-portable.json", sha256: malformedPortableSha256 };
    malformedPortableReview.roleProvenance.verifier.evidence = { kind: "artifact", path: "proof/malformed-portable.json", sha256: malformedPortableSha256 };
    const malformedPortablePayload = canonicalJson(reviewAttestationPayload(malformedPortableReview));
    malformedPortableReview.attestation.payloadSha256 = digest(malformedPortablePayload);
    malformedPortableReview.attestation.signature = signPayload(null, Buffer.from(malformedPortablePayload, "utf8"), privateKey).toString("base64");
    writeJson(path.join(root, "review", "malformed-portable.json"), malformedPortableReview);
    expectFailure(
      run("review-gate.mjs", ["--repo", root, "--file", "review/malformed-portable.json"], { cwd: root, scriptDir: reviewRuntimeScripts }),
    ...
    expectFailure(run("review-gate.mjs", ["--repo", root, "--file", "review/missing-scout.json"], { cwd: root, scriptDir: reviewRuntimeScripts }), "review without scout provenance", "roleProvenance.scout is required");

    const collapsedActor = structuredClone(reviewDocument);
    collapsedActor.roleProvenance.scout.actorId = collapsedActor.roleProvenance.implementer.actorId;
    const collapsedActorPayload = canonicalJson(reviewAttestationPayload(collapsedActor));
    collapsedActor.attestation.payloadSha256 = digest(collapsedActorPayload);
    collapsedActor.attestation.signature = signPayload(null, Buffer.from(collapsedActorPayload, "utf8"), privateKey).toString("base64");
    writeJson(path.join(root, "review", "collapsed-actor.json"), collapsedActor);
    expectFailure(run("review-gate.mjs", ["--repo", root, "--file", "review/collapsed-actor.json"], { cwd: root, scriptDir: reviewRuntimeScripts }), "review with collapsed scout and implementer actor", "actorId is reused");

    const collapsedSession = structuredClone(reviewDocument);
    collapsedSession.roleProvenance.verifier.sessionId = collapsedSession.roleProvenance.implementer.sessionId;
    const collapsedSessionPayload = canonicalJson(reviewAttestationPayload(collapsedSession));
    collapsedSession.attestation.payloadSha256 = digest(collapsedSessionPayload);
    collapsedSession.attestation.signature = signPayload(null, Buffer.from(collapsedSessionPayload, "utf8"), privateKey).toString("base64");
    writeJson(path.join(root, "review", "collapsed-session.json"), collapsedSession);
    expectFailure(run("review-gate.mjs", ["--repo", root, "--file", "review/collapsed-session.json"], { cwd: root, scriptDir: reviewRuntimeScripts }), "review with reused implementer and verifier session", "sessionId is reused");

    const collapsedExecution = structuredClone(reviewDocument);
    collapsedExecution.roleProvenance.independentReviewer.executionId = collapsedExecution.roleProvenance.scout.executionId;
    const collapsedExecutionPayload = canonicalJson(reviewAttestationPayload(collapsedExecution));
    collapsedExecution.attestation.payloadSha256 = digest(collapsedExecutionPayload);
    collapsedExecution.attestation.signature = signPayload(null, Buffer.from(collapsedExecutionPayload, "utf8"), privateKey).toString("base64");
    writeJson(path.join(root, "review", "collapsed-execution.json"), collapsedExecution);
    expectFailure(run("review-gate.mjs", ["--repo", root, "--file", "review/collapsed-execution.json"], { cwd: root, scriptDir: reviewRuntimeScripts }), "review with reused scout and independent-reviewer execution", "executionId is reused");

    const sameReviewerPath = path.join(root, "review", "same-reviewer.json");
    const sameReviewer = structuredClone(reviewDocument);
    sameReviewer.roleProvenance.independentReviewer.actorId = sameReviewer.roleProvenance.implementer.actorId;
    const sameReviewerPayload = canonicalJson(reviewAttestationPayload(sameReviewer));
    sameReviewer.attestation.payloadSha256 = digest(sameReviewerPayload);
    sameReviewer.attestation.signature = signPayload(null, Buffer.from(sameReviewerPayload, "utf8"), privateKey).toString("base64");
    writeJson(sameReviewerPath, sameReviewer);
    expectFailure(run("review-gate.mjs", ["--repo", root, "--file", "review/same-reviewer.json"], { cwd: root, scriptDir: reviewRuntimeScripts }), "same-actor review", "actorId is reused");
    ...
    forgedReview.roleProvenance.independentReviewer.actorId = "EXAMPLE-FORGED-REVIEWER";
    forgedReview.roleProvenance.independentReviewer.sessionId = "EXAMPLE-FORGED-SESSION";
    forgedReview.roleProvenance.independentReviewer.executionId = "EXAMPLE-FORGED-EXECUTION";
    ...
    console.log(JSON.stringify({ ok: true, seams: ["proof-runner CLI exit + JSON artifact", "RCA gate CLI exit", "review gate CLI exit"], proofCases: 18, rcaCases: 20, reviewCases: 10, adversarialRejections: 29 }, null, 2));
  } finally {

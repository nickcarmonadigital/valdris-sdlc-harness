...
import {
  ...
  routeRequiredGates,
  RUN_PACKET_SCHEMA,
  supportingArtifactsForGate,
  validateRunPacket,
  validationRuntimeBinding,
} from "./run-packet-gate.mjs";
import { ASSURANCE_LEVELS } from "./v09-assurance-lib.mjs";
...
function parseGate(value) {
  ...
  if (separator < 1 || separator === value.length - 1)
    throw new Error("--gate must use name=artifact/path.json");
  return {
    name: value.slice(0, separator),
    artifactPath: value.slice(separator + 1),
  };
}
...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    else if (arg === "--assurance-level") args.assuranceLevel = argv[++index];
    ...
    else if (arg === "--print-accepted-gate-set")
      args.printAcceptedGateSet = true;
    else if (arg === "--force")
      throw new Error("--force is disabled: run packets are immutable");
...
function usage() {
  return "Usage: node scripts/run-create.mjs --repo . --run-id ID --commit SHA --environment NAME --proof proof/portable.json [--assurance-level structural|semantic|authoritative] [--review review/review.json | --print-evidence-bundle | --print-accepted-gate-set] [--rca rca/rca.json] [--gate name=path] [--intake run/intake.json] [--route run/route.json] [--classification run/workload-classification.json] [--goal goal/goal.json] [--output run/packet.json]";
}
  ...
  const visit = (directory) => {
    for (const entry of readdirSync(directory, { withFileTypes: true })) {
      ...
      if (stats.isSymbolicLink())
        throw new Error(
          `artifact inventory must not contain symbolic links: ${relativePath}`,
        );
      if (stats.isDirectory()) visit(target);
      else if (
        stats.isFile() &&
        !excluded.has(relativePath) &&
        !isTracked(repoRoot, relativePath)
      ) {
        if (stats.size > MAX_INVENTORY_FILE_BYTES)
          throw new Error(
            `artifact inventory file exceeds ${MAX_INVENTORY_FILE_BYTES} bytes: ${relativePath}`,
          );
        totalBytes += stats.size;
        if (totalBytes > MAX_INVENTORY_BYTES)
          throw new Error(
            `artifact inventory exceeds ${MAX_INVENTORY_BYTES} bytes`,
          );
        inventory.push({
          path: relativePath,
        ...
        if (inventory.length > MAX_INVENTORY_FILES)
          throw new Error(
            `artifact inventory exceeds ${MAX_INVENTORY_FILES} files`,
          );
      }
    }
  };
...
function main() {
  ...
  if (!safeIdentifier(args.commit))
    throw new Error("--commit must be a safe identifier");
  if (!safeIdentifier(args.environment))
    throw new Error("--environment must be a safe identifier");
  if (!ASSURANCE_LEVELS.includes(args.assuranceLevel))
    throw new Error(
      "--assurance-level must be structural, semantic, or authoritative",
    );
  if (!args.proof) throw new Error("--proof is required");
  if (!args.review && !args.printEvidenceBundle && !args.printAcceptedGateSet)
    throw new Error(
      "--review is required unless --print-evidence-bundle is used",
    );
  ...
  const inputFiles = {
    ...
    classification: resolveArtifactPath(repoRoot, args.classification, {
      mustExist: true,
    }),
  ...
  const addGate = (name, artifactPath) => {
    ...
    if (suppliedGates.has(name))
      throw new Error(`gate was supplied more than once: ${name}`);
  ...
  for (const gate of requiredGates) {
    if (
      (args.printEvidenceBundle && gate === "independent-review") ||
      (args.printAcceptedGateSet &&
        ["independent-review", "authoritative-assurance"].includes(gate))
    )
      continue;
    if (!suppliedGates.has(gate))
      throw new Error(`required gate artifact was not supplied: ${gate}`);
  }
  for (const gate of suppliedGates.keys()) {
    if (!requiredGates.includes(gate))
      throw new Error(`gate is not required by this route: ${gate}`);
  }

  const gateArtifacts = requiredGates
    .filter((gate) => suppliedGates.has(gate))
    .map((gate) => {
      const file = resolveArtifactPath(repoRoot, suppliedGates.get(gate), {
        mustExist: true,
      });
      const artifact = {
        gate,
        path: normalizedRelative(repoRoot, file),
      ...
      if (supportingArtifacts.length > 0)
        artifact.supportingArtifacts = supportingArtifacts;
      return artifact;
    });
  const inventoryExclusions = [args.output, args.review]
    .filter(Boolean)
    .map((artifactPath) =>
      normalizedRelative(repoRoot, resolveArtifactPath(repoRoot, artifactPath)),
    );
  const validationRuntime = validationRuntimeBinding(repoRoot, args.commit);
  const packet = {
    ...
    inputs: Object.fromEntries(
      Object.entries(inputFiles).map(([name, file]) => [
        name,
        { path: normalizedRelative(repoRoot, file), sha256: fileSha256(file) },
      ]),
    ),
    validationRuntime,
    requiredGates,
    ...
    catalogSnapshots: resolvedCatalogSnapshots(undefined, validationRuntime),
  };
  if (args.printAcceptedGateSet)
    return console.log(
      JSON.stringify(
        {
          ok: true,
          acceptedGateArtifactsSha256: acceptedGateArtifactSetSha256(
      ...
      JSON.stringify(
        {
          ok: true,
          evidenceBundle,
          evidenceBundleSha256: reviewEvidenceBundleSha256(packet),
          acceptedGateArtifactsSha256: acceptedGateArtifactSetSha256(
            packet.gateArtifacts,
          ),
        },
  ...
  const reviewFile = resolveArtifactPath(
    repoRoot,
    suppliedGates.get("independent-review"),
    { mustExist: true },
  );
  packet.roleProvenanceSha256 = reviewRoleProvenanceSha256(
    readJson(reviewFile),
  );
  ...
  if (!validation.valid)
    throw new Error(
      `refusing to create invalid run packet: ${validation.problems.join("; ")}`,
    );

  const output = resolveArtifactPath(repoRoot, args.output);
  if (existsSync(output))
    throw new Error("run packets are immutable; choose a new output path");
  ...
  console.log(
    JSON.stringify(
      {
        ok: true,
        artifact: normalizedRelative(repoRoot, output),
        runId: packet.runId,
        requiredGates: packet.requiredGates,
        binding: packet.bindings.envelopeSha256,
      },
      null,
      2,
    ),

...
function appendYamlList(lines, key, values, indent = 4) {
  ...
  lines.push(`${" ".repeat(indent)}${key}:`);
  for (const value of values || []) lines.push(`${" ".repeat(indent + 2)}- ${yamlString(value)}`);
}
...
export function renderCodexRoutingYaml(document, repoRoot) {
  ...
  for (const skill of document.skills || []) {
    ...
    const metadata = target && existsSync(target) ? frontmatter(readFileSync(target, "utf8")) : {};
...
function validateSkillMirror(document, repoRoot, relativeRoot, problems) {
  ...
  for (const entry of readdirSync(mirrorRoot, { withFileTypes: true })) {
    if (entry.isDirectory() && entry.name.startsWith("valdris-") && !expected.has(entry.name)) problems.push(`${relativeRoot} contains unregistered Valdris skill: ${entry.name}`);
  }
  for (const skill of document.skills || []) {
    const canonicalTarget = resolveWithinRepo(repoRoot, nonEmpty(skill.path));
    if (!canonicalTarget || !existingFileWithinRepo(repoRoot, canonicalTarget)) continue;
    const canonicalDir = path.dirname(canonicalTarget);
    for (const relativeFile of ["SKILL.md", path.join("agents", "openai.yaml")]) {
      const canonical = path.join(canonicalDir, relativeFile);
      const mirror = path.join(mirrorRoot, skill.name, relativeFile);
      if (!existingFileWithinRepo(repoRoot, mirror)) problems.push(`${relativeRoot}/${skill.name}/${relativeFile.replaceAll("\\", "/")} is missing or outside repo`);
      else if (readFileSync(mirror, "utf8") !== readFileSync(canonical, "utf8")) problems.push(`${relativeRoot}/${skill.name}/${relativeFile.replaceAll("\\", "/")} differs from canonical skill`);
    }
  }
  for (const file of ["registry.json", "codex-routing.yaml"]) {
    ...
    if (!existingFileWithinRepo(repoRoot, mirror)) problems.push(`${relativeRoot}/${file} is missing or outside repo`);
    else if (readFileSync(mirror, "utf8") !== readFileSync(canonical, "utf8")) problems.push(`${relativeRoot}/${file} differs from canonical skills/${file}`);
  }
}
...
export function validateSkillRegistry(document, repoRoot) {
  ...
  for (const skill of document.skills || []) {
    ...
    const openAiTarget = path.join(path.dirname(target), "agents", "openai.yaml");
    if (!existingFileWithinRepo(repoRoot, openAiTarget)) {
      problems.push(`skill ${name} agents/openai.yaml is missing or outside repo`);
    } else {
      const openAiYaml = readFileSync(openAiTarget, "utf8");
      const displayName = quotedInterfaceField(openAiYaml, "display_name");
      const shortDescription = quotedInterfaceField(openAiYaml, "short_description");
      const defaultPrompt = quotedInterfaceField(openAiYaml, "default_prompt");
      if (!displayName) problems.push(`skill ${name} agents/openai.yaml display_name is required`);
      if (shortDescription.length < 25 || shortDescription.length > 64) problems.push(`skill ${name} agents/openai.yaml short_description must be 25-64 characters`);
      if (!defaultPrompt.includes(`$${name}`)) problems.push(`skill ${name} agents/openai.yaml default_prompt must mention $${name}`);
      if (!/^policy:\r?\n  allow_implicit_invocation:\s*true\s*$/m.test(openAiYaml)) problems.push(`skill ${name} must allow implicit invocation in agents/openai.yaml`);
    }
  ...
  if (!existingFileWithinRepo(repoRoot, routingTarget)) {
    ...
    if (readFileSync(routingTarget, "utf8").replace(/\r\n/g, "\n") !== expectedRouting) problems.push("skills/codex-routing.yaml is stale relative to skills/registry.json or SKILL.md metadata");
  }
  ...
  const looksCommissioned = existsSync(path.join(repoRoot, "00_MAP.md")) || existsSync(path.join(repoRoot, "CONTEXT.md")) || existsSync(path.join(repoRoot, "commissioning-review.md"));
  ...
  if (existsSync(adapterTarget)) {
    ...
    if (adapter.schema !== "uash.project-adapter.v2") {
      ...
      if (adapter.skillRouter?.registry !== "skills/registry.json") problems.push("project adapter skillRouter.registry must be skills/registry.json");
      if (adapter.skillRouter?.codexRouting !== "skills/codex-routing.yaml") problems.push("project adapter skillRouter.codexRouting must be skills/codex-routing.yaml");
      if (adapter.skillRouter?.implicitInvocation !== true) problems.push("project adapter skillRouter.implicitInvocation must be true");
    }
  }
  if ((hasAgentMirror || hasClaudeMirror || looksCommissioned) && !existsSync(adapterTarget)) problems.push("project-adapter.json is required when generated skill mirrors or commissioned front doors are present");
  if (hasAgentMirror || hasClaudeMirror || looksCommissioned || adapterRequiresMirrors) {
    validateSkillMirror(document, repoRoot, path.join(".agents", "skills"), problems);
    validateSkillMirror(document, repoRoot, path.join(".claude", "skills"), problems);
  }
  const metaSkillTarget = path.join(repoRoot, "meta-skills", "valdris-sdlc-harness", "SKILL.md");
  if (existsSync(metaSkillTarget)) {
    const metadata = frontmatter(readFileSync(metaSkillTarget, "utf8"));
    const metaOpenAi = path.join(path.dirname(metaSkillTarget), "agents", "openai.yaml");
    if (metadata.name !== "valdris-sdlc-harness" || !nonEmpty(metadata.description)) problems.push("meta skill valdris-sdlc-harness has invalid YAML frontmatter");
    if (!existingFileWithinRepo(repoRoot, metaOpenAi) || !/^policy:\r?\n  allow_implicit_invocation:\s*true\s*$/m.test(readFileSync(metaOpenAi, "utf8"))) problems.push("meta skill valdris-sdlc-harness must allow implicit invocation");
  }

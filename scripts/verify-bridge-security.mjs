...
for (const [index, name] of BRIDGE_CREDENTIAL_ENV_NAMES.entries()) {
  ...
  contaminatedEnvironment[name.toLowerCase()] = [
    "lowercase",
    "fixture",
    index,
  ].join("-");
  contaminatedEnvironment[alternatingCase(name)] = [
    "mixed-case",
    "fixture",
    index,
  ].join("-");
...
const credentialAliases = (environment) =>
  Object.keys(environment).filter((name) =>
    credentialNames.has(name.toUpperCase()),
  );

const childEnvironment = finishLineChildEnv(contaminatedEnvironment);
assert.deepEqual(
  credentialAliases(childEnvironment),
  [],
  "sanitized validator environment retained a case-variant bridge credential",
);
assert.equal(
  childEnvironment.UASH_REVIEW_TRUST_SHA256,
  reviewTrustSha256,
  "sanitization removed the nonsecret review-trust pin",
);
...
const child = spawnSync(
  ...
  [
    ...
    JSON.stringify([...credentialNames]),
  ],
  {
    encoding: "utf8",
...
assert.equal(
  child.status,
  0,
  child.stderr || child.error?.message || "validator child failed",
);
const observed = JSON.parse(child.stdout);
assert.deepEqual(
  observed.aliases,
  [],
  "validator child inherited a case-variant bridge credential",
);
assert.equal(
  observed.reviewTrustSha256,
  reviewTrustSha256,
  "validator child did not inherit the nonsecret review-trust pin",
);
...
const tempRoot = await mkdtemp(
  path.join(os.tmpdir(), "valdris-bridge-security-"),
);
...
async function waitForBridge() {
  ...
  while (Date.now() - started < 5000) {
    if (bridge.exitCode !== null)
      throw new Error(`bridge exited during startup: ${bridgeLog}`);
    try {
...
try {
  ...
  assert.equal(
    JSON.parse(invalidListPage.text).error,
    "invalid_pagination",
    "list endpoint accepted an out-of-range page limit",
  );
  const unknownListPage = await get("/runs?eventLimit=1");
  assert.equal(unknownListPage.status, 400, unknownListPage.text);
  assert.equal(
    JSON.parse(unknownListPage.text).error,
    "invalid_pagination",
    "list endpoint accepted a detail-only query parameter",
  );

  const pagedRunId = "VERIFY-PAGED-HISTORY";
  const pagedCreated = await post("/runs", {
    id: pagedRunId,
    title: "bounded event-history fixture",
  });
  ...
  const countLimited = await post(`/runs/${pagedRunId}/events`, {
    ...
    actor: "codex",
    message:
      "this event must not be persisted beyond the configured run-history limit",
    status: "ok",
    runMode: "live",
    eventSource: "bridge",
  });
  ...
  assert.equal(
    JSON.parse(countLimited.text).error,
    "event_state_violation",
    "run event-count limit did not fail at the state boundary",
  );

  const firstEventPage = await get(
    `/runs/${pagedRunId}?eventLimit=2&eventCursor=0`,
  );
  assert.equal(firstEventPage.status, 200, firstEventPage.text);
  const firstPageDocument = JSON.parse(firstEventPage.text);
  assert.deepEqual(
    firstPageDocument.events.map(({ id }) => id),
    [`${pagedRunId}-0`, `${pagedRunId}-1`],
    "detail event cursor did not return the requested stable event slice",
  );
  assert.deepEqual(
    firstPageDocument.eventPage,
    {
      offset: 0,
      limit: 2,
      returned: 2,
  ...
  assert.equal(
    firstEventPage.headers.get("x-uash-next-cursor"),
    "2",
    "detail response omitted its next cursor header",
  );
  assert.ok(
    Number(firstEventPage.headers.get("content-length")) <= 768 * 1024,
    "detail response exceeded the bridge byte cap",
  );
  ...
  assert.deepEqual(
    newestPageDocument.events.map(({ id }) => id),
    [`${pagedRunId}-2`, `${pagedRunId}-3`],
    "detail default did not retain the newest bounded event page",
  );
  assert.equal(
    newestPageDocument.eventPage.offset,
    2,
    "newest detail page reported the wrong event offset",
  );
  ...
  assert.equal(
    JSON.parse(invalidEventPage.text).error,
    "invalid_pagination",
    "detail endpoint accepted an out-of-range event limit",
  );
  const unknownEventPage = await get(`/runs/${pagedRunId}?limit=1`);
  assert.equal(unknownEventPage.status, 400, unknownEventPage.text);
  assert.equal(
    JSON.parse(unknownEventPage.text).error,
    "invalid_pagination",
    "detail endpoint accepted a list-only query parameter",
  );
  ...
  assert.equal(
    listDocument.length,
    1,
    "list endpoint ignored its bounded run limit",
  );
  assert.deepEqual(
    listDocument[0].events,
    [],
    "list summary exposed an event history",
  );
  assert.equal(
    typeof listDocument[0].eventCount,
    "number",
    "list summary omitted the persisted event count",
  );
  assert.equal(
    listPage.headers.get("x-uash-page-limit"),
    "1",
    "list response omitted its page-limit header",
  );
  assert.ok(
    Number(listPage.headers.get("content-length")) <= 768 * 1024,
    "list response exceeded the bridge byte cap",
  );

  const approvalRunId = "VERIFY-BOUNDED-APPROVALS";
  const approvalCreated = await post("/runs", {
    id: approvalRunId,
    title: "bounded approval-history fixture",
  });
  ...
  assert.equal(
    boundedApprovalDocument.approvalCount,
    4,
    "detail response lost the authoritative approval count",
  );
  assert.equal(
    boundedApprovalDocument.approvalsTruncated,
    true,
    "detail response did not byte-bound a large valid approval history",
  );
  assert.ok(
    boundedApprovalDocument.approvals.length <
      boundedApprovalDocument.approvalCount,
    "detail response returned the entire oversized approval projection",
  );
  const boundedApprovalList = await get("/runs?limit=50");
  assert.equal(boundedApprovalList.status, 200, boundedApprovalList.text);
  const boundedApprovalSummary = JSON.parse(boundedApprovalList.text).find(
    (run) => run.id === approvalRunId,
  );
  assert.ok(
    boundedApprovalSummary &&
      boundedApprovalSummary.approvalsTruncated === true,
    "one large approval history poisoned or disappeared from the global run list",
  );
  ...
  assert.equal(
    JSON.parse(oversizedEvent.text).error,
    "event_contract_violation",
    "oversized event input was not rejected before journal append",
  );

  const oversizedStateRunId = "VERIFY-OVERSIZED-STATE";
  const oversizedStateCreated = await post("/runs", {
    id: oversizedStateRunId,
  });
  ...
  assert.match(
    JSON.parse(oversizedSnapshotRead.text).error,
    /bridge run snapshot must be a regular file no larger than/,
    "oversized snapshot was parsed instead of rejected by stat size",
  );
  assert.ok(
    Number(oversizedSnapshotRead.headers.get("content-length")) <= 768 * 1024,
    "oversized snapshot error exceeded the response cap",
  );
  ...
  assert.match(
    JSON.parse(oversizedConfigRead.text).error,
    /bridge run configuration must be a regular file no larger than/,
    "oversized run configuration was parsed instead of rejected by stat size",
  );
  await writeFile(configPath, trustedConfig);

  const malformedMarker = "SENSITIVE-MALFORMED-BODY-MARKER";
  const malformed = await rawPost("/runs", [
    `{\"id\":\"${malformedMarker}\",}`,
  ]);
  assert.equal(malformed.status, 400, malformed.text);
  assert.deepEqual(
    JSON.parse(malformed.text),
    { ok: false, error: "invalid_json" },
    "malformed JSON must return only a generic client error",
  );
  assert.equal(
    malformed.text.includes(malformedMarker),
    false,
    "malformed JSON response echoed request-body bytes",
  );
  ...
  for (const pathname of ["/runs", "/runs/VERIFY-JSON-SHAPE/events"]) {
    for (const invalidDocument of ["null", "[]"]) {
      ...
      assert.equal(
        invalidShape.status,
        400,
        `${pathname} accepted a non-object JSON document: ${invalidShape.text}`,
      );
      assert.deepEqual(
        JSON.parse(invalidShape.text),
        { ok: false, error: "invalid_json" },
        `${pathname} must reject non-object JSON generically`,
      );
    }
  }
  ...
  assert.deepEqual(
    JSON.parse(declaredOversize.text),
    { ok: false, error: "payload_too_large" },
    "declared oversized body must return only a generic 413",
  );

  const chunkedOversize = await rawPost(
    "/runs",
    [
  ...
  assert.deepEqual(
    JSON.parse(chunkedOversize.text),
    { ok: false, error: "payload_too_large" },
    "chunked oversized body must return only a generic 413",
  );
  ...
  for (const field of ["humanToken", "humanApprovalToken"]) {
    ...
    assert.equal(
      rejected.status,
      400,
      `${field} plus a valid header was accepted: ${rejected.text}`,
    );
    assert.equal(
      rejected.text.includes(humanApprovalToken),
      false,
      `${field} rejection echoed the raw approval credential`,
    );
  }
} finally {
  await stopBridge();
...
const emitterDiagnostic = spawnSync(
  process.execPath,
  [
...
assert.notEqual(
  emitterDiagnostic.status,
  0,
  "operator emitter accepted the removed human-token argv channel",
);
assert.equal(
  emitterOutput.includes(legacyHumanToken),
  false,
  "operator emitter diagnostic echoed a raw equals-form human token",
);
assert.match(
  emitterOutput,
  /Unknown option: --human-token/,
  "operator emitter diagnostic omitted the rejected flag name",
);

const leadingEmitterDiagnostic = spawnSync(
  process.execPath,
  [
    "scripts/uash-emit-event.mjs",
    "VERIFY-DIAGNOSTIC",
...
assert.notEqual(
  leadingEmitterDiagnostic.status,
  0,
  "operator emitter treated a leading removed human-token option as message text",
);
assert.equal(
  leadingEmitterOutput.includes(legacyHumanToken),
  false,
  "operator emitter echoed a leading raw human token",
);
assert.match(
  leadingEmitterOutput,
  /Unknown option: --human-token/,
  "operator emitter did not identify the rejected leading flag safely",
);
...
for (const optionTail of [
  ...
  const malformedKnownOption = spawnSync(
    process.execPath,
    [
      "scripts/uash-emit-event.mjs",
      "VERIFY-DIAGNOSTIC",
  ...
  assert.notEqual(
    malformedKnownOption.status,
    0,
    "operator emitter accepted a missing or flag-like known-option value",
  );
  assert.equal(
    malformedOutput.includes(legacyHumanToken),
    false,
    "known-option diagnostic echoed the consumed flag-like secret value",
  );
  assert.match(
    malformedOutput,
    /Option --approval-owner requires a non-empty, non-flag value/,
    "operator emitter did not fail at the malformed known option",
  );
}

console.log(
  "bridge security verification passed: credentials isolated, request/response/state bytes bounded, histories paged and count-limited, malformed inputs redacted, and approval credentials restricted to headers",
);

...
const DOCUMENTATION_ROOT_FILES = new Set(["readme.md", "claude.md", "agents.md"]);
const DOCUMENTATION_DIRECTORY_SEGMENTS = new Set(["doc", "docs", "playbook", "playbooks", "process", "processes"]);
...
function git(repoRoot, args, options = {}) {
  return spawnSync("git", args, { cwd: repoRoot, encoding: options.encoding ?? "utf8", shell: false, timeout: 30_000, killSignal: "SIGTERM", stdio: ["ignore", "pipe", "pipe"] });
}
...
function documentationProcessPath(value) {
  ...
  return parts.slice(0, -1).some((segment) => DOCUMENTATION_DIRECTORY_SEGMENTS.has(segment.toLowerCase()));
}

function targetGitPrefix(repoRoot) {
  const result = git(repoRoot, ["rev-parse", "--show-prefix"]);
  if (result.status !== 0) throw new Error("RCA could not resolve the target Git prefix");
  const prefix = result.stdout.replace(/\r?\n$/, "").replaceAll("\\", "/");
  if (prefix === ".." || prefix.startsWith("../") || path.posix.isAbsolute(prefix)) throw new Error("RCA target Git prefix is invalid");
  return prefix;
}

function committedRegularFileSha256(repoRoot, revision, relativePath) {
  const gitPath = `${targetGitPrefix(repoRoot)}${relativePath}`;
  const tree = git(repoRoot, ["ls-tree", "--full-name", "-z", revision, "--", `:(top,literal)${gitPath}`], { encoding: null });
  if (tree.status !== 0) throw new Error(`cannot inspect ${relativePath} at ${revision}`);
  const record = Buffer.from(tree.stdout || []).toString("utf8").split("\0").filter(Boolean);
  if (record.length !== 1) throw new Error(`causal input must exist exactly once at ${revision}: ${relativePath}`);
  ...
  if (!new Set(["100644", "100755"]).has(header[0]) || header[1] !== "blob" || recordedPath !== gitPath) {
    throw new Error(`causal input must be a regular non-symlink Git file at ${revision}: ${relativePath}`);
  }
  const blob = git(repoRoot, ["cat-file", "blob", `${revision}:${gitPath}`], { encoding: null });
  if (blob.status !== 0) throw new Error(`cannot read ${relativePath} at ${revision}`);
...
function regressionCausalInputState(document, repoRoot) {
  ...
  if (declared === undefined) return { entries: [], byPath: new Map(), problems };
  if (!Array.isArray(declared) || declared.length === 0) return { entries: [], byPath: new Map(), problems: ["RCA regression.causalInputs must be a non-empty array when supplied"] };
  ...
  for (const [index, entry] of declared.entries()) {
    ...
    if (canonicalJson(Object.keys(entry).sort()) !== canonicalJson(["path", "postFixSha256", "preFixSha256"])) problems.push(`${label} must contain exactly path, preFixSha256, and postFixSha256`);
    try { assertCanonicalRepoRelativePath(entry.path, `${label}.path`); }
    catch (error) { problems.push(error.message); }
    if (typeof entry.path === "string") {
      if (byPath.has(entry.path)) problems.push(`${label}.path is duplicated`);
      else byPath.set(entry.path, entry);
    }
    if (!SHA256.test(entry.preFixSha256 || "")) problems.push(`${label}.preFixSha256 is invalid`);
    if (!SHA256.test(entry.postFixSha256 || "")) problems.push(`${label}.postFixSha256 is invalid`);
    if (SHA256.test(entry.preFixSha256 || "") && SHA256.test(entry.postFixSha256 || "") && entry.preFixSha256 === entry.postFixSha256) problems.push(`${label} pre-fix and post-fix digests must differ`);
    if (/^[a-f0-9]{40,64}$/i.test(document.regression?.preFixCommit || "") && SHA256.test(entry.preFixSha256 || "")) {
      try {
        if (committedRegularFileSha256(repoRoot, document.regression.preFixCommit, entry.path) !== entry.preFixSha256) problems.push(`${label}.preFixSha256 does not match the exact pre-fix Git bytes`);
      } catch (error) { problems.push(`${label} pre-fix binding is invalid: ${error.message}`); }
    }
    if (/^[a-f0-9]{40,64}$/i.test(document.regression?.postFixCommit || "") && SHA256.test(entry.postFixSha256 || "")) {
      try {
        if (committedRegularFileSha256(repoRoot, document.regression.postFixCommit, entry.path) !== entry.postFixSha256) problems.push(`${label}.postFixSha256 does not match the exact post-fix Git bytes`);
      } catch (error) { problems.push(`${label} post-fix binding is invalid: ${error.message}`); }
    }
  }
  const normalized = [...declared].sort((left, right) => String(left?.path || "") < String(right?.path || "") ? -1 : String(left?.path || "") > String(right?.path || "") ? 1 : 0);
  if (canonicalJson(declared) !== canonicalJson(normalized)) problems.push("RCA regression.causalInputs must use canonical path order");
  return { entries: declared, byPath, problems };
}

function causalProofBindingProblems(document, reproductionProof, regressionProof) {
  const problems = [];
  const declared = Array.isArray(document.regression?.causalInputs) ? document.regression.causalInputs : [];
  for (const entry of declared) {
    ...
    for (const [phase, proof, expectedDigest] of [
      ...
      if (!proof) continue;
      const bound = (proof.causalInputs || []).find((candidate) => candidate?.path === entry.path);
      if (!bound) {
        problems.push(`RCA ${phase} proof is missing digest-bound causal input: ${entry.path}`);
        continue;
      }
      if (bound.beforeSha256 !== expectedDigest || bound.afterSha256 !== expectedDigest) {
        problems.push(`RCA ${phase} proof causal input does not match the exact ${phase === "reproduction" ? "pre-fix" : "post-fix"} Git bytes: ${entry.path}`);
      }
    }
  }
  return problems;
...
function revisionProblems(document, repoRoot) {
  ...
  if (/^[a-f0-9]{40,64}$/i.test(preFixCommit || "") && /^[a-f0-9]{40,64}$/i.test(postFixCommit || "") && preFixCommit !== postFixCommit) {
    ...
    const targetPathspec = targetPrefix ? `:(top,literal)${targetPrefix.replace(/\/$/, "")}` : ".";
    const changed = git(repoRoot, ["diff", "--name-only", "-z", preFixCommit, postFixCommit, "--", targetPathspec]);
    if (changed.status !== 0) problems.push("RCA could not verify the pre-fix to post-fix source change");
    else {
      const changedPaths = [];
      for (const entry of changed.stdout.split("\0").filter(Boolean).map((value) => value.replaceAll("\\", "/"))) {
        if (targetPrefix && !entry.startsWith(targetPrefix)) {
          problems.push(`RCA pre/post diff escaped the commissioned target prefix: ${entry}`);
          continue;
        }
        const targetRelative = targetPrefix ? entry.slice(targetPrefix.length) : entry;
      ...
      if (causalFixPaths.length === 0) problems.push("RCA root cause and fix must overlap on a changed source path");
        ...
        if (causalClass !== undefined && !CAUSAL_CLASSES.has(causalClass)) problems.push("RCA rootCause.causalClass must be source-change or documentation-process when supplied");
        if (remediationClass !== undefined && !CAUSAL_CLASSES.has(remediationClass)) problems.push("RCA fix.remediationClass must be source-change or documentation-process when supplied");
        if (causalClass !== undefined && remediationClass !== undefined && causalClass !== remediationClass) problems.push("RCA rootCause.causalClass and fix.remediationClass must match");
        const nonEvidenceCausalPaths = causalFixPaths.filter((value) => !isHarnessEvidencePath(value));
        const documentationCausalPaths = nonEvidenceCausalPaths.filter(documentationProcessPath);
        const sourceCausalPaths = nonEvidenceCausalPaths.filter((value) => !documentationProcessPath(value));
        for (const entry of causalInputs.entries) {
          if (typeof entry?.path === "string" && !causalFixPaths.includes(entry.path)) problems.push(`RCA regression.causalInputs path must be a declared causal changed path: ${entry.path}`);
        }
        for (const causalPath of documentationCausalPaths) {
          if (!causalInputs.byPath.has(causalPath)) problems.push(`RCA documentation/process causal path is missing from regression.causalInputs: ${causalPath}`);
        }
        if (sourceCausalPaths.length === 0 && documentationCausalPaths.length === 0) {
          problems.push("RCA causal fix path must itself be a changed non-evidence source path or a typed documentation-process path");
        } else if (sourceCausalPaths.length === 0) {
          if (causalClass !== "documentation-process" || remediationClass !== "documentation-process") {
            problems.push("RCA documentation-only causal fix requires rootCause.causalClass and fix.remediationClass to be documentation-process");
          }
        } else if (causalClass === "documentation-process" || remediationClass === "documentation-process") {
          problems.push("RCA documentation-process classification cannot be used when the causal fix includes a non-documentation source path");
        }
      }
    }
  }
...
export function validateRcaArtifact(document, repoRoot) {
    ...
    try {
      ...
      if (evidence.type === "runtime-command") {
        ...
        if (["reproduction", "post-fix-regression"].includes(evidence.phase)
          && (proof.command?.executionInputs?.stability !== "static" || proof.command?.executionInputs?.causalIdentityEligible !== true)) {
          problems.push(`${label} opaque dynamic execution inputs are ineligible for RCA causal identity`);
        }
  ...
  problems.push(...causalProofBindingProblems(document, reproductionProof, regressionProof));

...
const CONNECTION_CREDENTIAL_PATTERN = /\b(?:postgres(?:ql)?|mysql|mariadb|mongodb(?:\+srv)?|redis(?:s)?|mssql):\/\/([^:\s"'`@\/]*):([^@\s"'`\/]+)@[^\s"'`<>]+/gi;
...
const CONTENT_DETECTORS = [
  ...
  { id: "github-fine-grained-token", category: "secret", pattern: /\bgithub_pat_[A-Za-z0-9_]{20,255}\b/g },
  ...
  { id: "bearer-token", category: "secret", pattern: /\bBearer\s+[A-Za-z0-9._~+/=-]{16,}\b/gi },
  {
    ...
    category: "secret",
    pattern: /(?<![A-Za-z0-9])(?:[A-Za-z0-9]+[_-])*(?:access[_-]?token|refresh[_-]?token|api[_-]?key|auth|authorization|client[_-]?secret|private[_-]?key|password|credentials?|secret|token|dsn|connection[_-]?(?:string|url)|(?:database|db|postgres(?:ql)?|mysql|mariadb|mongo(?:db)?|redis|cache|mssql)[_-]?(?:url|dsn))\b\s*["']?\s*[:=]\s*(?:(?:\\+)?["'])?((?:<[^>\r\n]+>|\[[^\]\r\n]+\]|\$\{\{[^}\r\n]+\}\}|\$\{[^}\r\n]+\}|[^\s\\"',;}]{8,}))/gi,
  },
  { id: "connection-credential", category: "secret", pattern: CONNECTION_CREDENTIAL_PATTERN },
  { id: "windows-user-path", category: "local-user-path", pattern: /\b[A-Za-z]:(?:\\{1,4}|\/)Users(?:\\{1,4}|\/)[^\s"'`,;]+/gi },
  { id: "unix-user-path", category: "local-user-path", pattern: /(?<![A-Za-z0-9._~\/-])(?:\/(?:Users|home)\/[^/\s"'`,;]+(?:\/[^\s"'`,;]*)?|\/r[o]ot(?=\/|[\s"'`,;<>()\[\]{}]|$)(?:\/[^\s"'`,;<>()\[\]{}]*)?)/gi },
...
const RELEASE_ARTIFACT_DETECTORS = CONTENT_DETECTORS.filter((detector) => RELEASE_ARTIFACT_DETECTOR_IDS.has(detector.id));
...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    if (argv[index] === "--repo") args.repo = argv[++index];
      ...
      if (args.releaseArtifact !== null) throw new Error("--release-artifact may only be provided once");
...
function isReleaseArtifactGeneratedNoise(relativeToArtifact, entry) {
  ...
  if (entry.isDirectory()) {
    if (segments.length === 1 && RELEASE_ARTIFACT_EXCLUDED_ROOT_DIRS.has(segments[0])) return true;
    if (segments[0] === "standalone" && segments[1] === "node_modules") return true;
    if (entry.name === "cache" && segments.at(-2) === ".next") return true;
    return false;
  }
  return RELEASE_ARTIFACT_EXCLUDED_FILE_PATTERNS.some((pattern) => pattern.test(entry.name));
}

function isExpectedReleaseBinaryAsset(relativePath) {
  return RELEASE_ARTIFACT_BINARY_EXTENSIONS.has(path.extname(relativePath).toLowerCase());
}
...
function collectReleaseArtifactFiles(repo, releaseArtifact) {
  ...
  const stats = lstatSync(resolved.target);
  if (stats.isSymbolicLink()) throw new Error("privacy release artifact root must not be a symbolic link");
  if (!stats.isDirectory()) throw new Error("privacy release artifact must resolve to a directory");
...
function isNonMaterialSecretReference(value) {
  ...
  return /^(?:undefined|null|true|false|configured|required|missing)$/i.test(normalized)
    || /^(?:[A-Za-z_$][A-Za-z0-9_$]*)(?:(?:\??\.)[A-Za-z_$][A-Za-z0-9_$]*|\[[A-Za-z0-9_$.'"`-]+\])*$/i.test(normalized)
    || /^(?:[A-Za-z_$][A-Za-z0-9_$]*)(?:(?:\??\.)[A-Za-z_$][A-Za-z0-9_$]*)*\([^"'`]*\)$/i.test(normalized);
}

function hasSafeAssignmentTerminator(match) {
  const suffix = String(match.input || "").slice((match.index || 0) + match[0].length);
  return /^(?:(?:\\+)?["'])?[ \t]*(?:[,;}\]]|#|$)/.test(suffix);
}
...
function isAllowedMatch(detector, match, relativePath) {
  ...
  if (detector.id === "secret-assignment") {
    ...
    if (isExactSecretPlaceholder(assignedValue) && (hasSafeAssignmentTerminator(match) || verificationFixture)) return true;
    if (isNonMaterialSecretReference(assignedValue) && (hasSafeAssignmentTerminator(match) || verificationFixture)) return true;
    const connectionMatch = allMatches(CONNECTION_CREDENTIAL_PATTERN, assignedValue)[0];
    if (connectionMatch && isExactSecretPlaceholder(connectionMatch[2]) && (hasSafeAssignmentTerminator(match) || verificationFixture)) return true;
    return verificationFixture && /^(?:fixture|synthetic|verify|example)-[a-z0-9_-]+$/i.test(assignedValue || "");
  }
  if (detector.id === "connection-credential") return isExactSecretPlaceholder(match[2]);
...
export function validateReleaseArtifactPrivacy(repo, releaseArtifact) {
  ...
  for (const target of collected.files) {
    ...
    if (isCompressedReleaseSourceMap(relativePath)) {
      ...
      findings.push(redactedFinding("unscannable-release-source-map", relativePath, 0, `${content.digest}:${content.size}`));
      continue;
    }
    if (content.binary) {
      if (isExpectedReleaseBinaryAsset(relativePath)) {
        ...
        findings.push(redactedFinding("unscannable-release-binary", relativePath, 0, `${content.digest}:${content.size}`));
      }
      continue;
    }
    scannedFiles += 1;
    ...
    for (let index = 0; index < lines.length; index += 1) {
      ...
      for (const detector of RELEASE_ARTIFACT_DETECTORS) {
        for (const match of allMatches(detector.pattern, line)) {
          if (isAllowedReleaseArtifactMatch(detector, match, relativePath)) continue;
          findings.push(redactedFinding(detector.category, relativePath, index + 1, match[0]));
        }
      }
    }
  ...
  return {
    ...
    detectorPolicy: "high-confidence-credentials-and-deployable-local-user-paths",
    binaryPolicy: "allow-known-release-asset-extensions-reject-compressed-source-maps-and-fail-closed-otherwise",
    sourceMapPolicy: "scan-plain-source-maps-and-reject-compressed-source-maps-without-decompression",
    generatedNoisePolicy: "exclude-next-cache-dev-diagnostics-types-traces-nft-standalone-dependencies-and-required-server-build-root-paths; scan-source-maps",
    symlinkFiles: collected.symlinks.length,
    scope: {
...
async function main() {
  ...
  if (args.help) {
    console.log("Usage: node scripts/privacy-gate.mjs --repo . [--include relative/file-or-directory ... | --release-artifact relative/directory]");
    return;
  }
  const output = args.releaseArtifact === null
    ? validatePrivacy(path.resolve(args.repo), { includes: args.includes })
    : validateReleaseArtifactPrivacy(path.resolve(args.repo), args.releaseArtifact);
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  main().catch((error) => {
    const gate = process.argv.includes("--release-artifact") ? "release-artifact-privacy" : "import-privacy";
    console.log(JSON.stringify({ ok: false, gate, findings: [{ category: "gate-error", path: "[REDACTED_PATH]", line: 0, redacted: "[REDACTED:GATE_ERROR]", fingerprint: createHash("sha256").update(error.name).digest("hex").slice(0, 12) }] }, null, 2));
    process.exitCode = 1;
  });
}

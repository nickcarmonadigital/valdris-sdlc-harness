...
export const CANONICAL_CATALOG_SHA256 = Object.freeze({
  "controls/workload-taxonomy.v1.json": "1d43a946a5aabe9919d46cdc52591950e85aa238e3d3886c3e6a2d65f157da14",
  "controls/foundation-layer.v1.json": "a7403ada1c5dad44e126544337a5c2884ac1eaac43e6f6c2daf1bb2df90b319e",
  "controls/production-layers.v2.json": "19bbb930c70eb99093fd2318b9fb45e4fc5543e8158c9c5d2e893c0d708ec3e5",
  "controls/genai-assurance.v1.json": "dbdfe7854f000dd6312f0c7c08fc882ec1b39bec411de9ccd52a423c3a358ebc",
  "controls/domain-packs/index.json": "993b99ac179e6220a7d26c86181babf6c79fb37db55f760e277d50e5777d6324",
  "controls/domain-packs/saas.v1.json": "4290d854d35217e9cc304a101ec53f378c24123407cd604229e0409da728b913",
  "controls/domain-packs/mobile-ios.v1.json": "c7ec574e021f25d54bd71d0717b4cf9abc675c753cae4ef1524da93848eeda28",
...
export function validateCatalogIntegrity(assetRoot = ASSET_ROOT) {
  const problems = [];
  for (const [relativePath, expected] of Object.entries(CANONICAL_CATALOG_SHA256)) {
    ...
    try {
      if (semanticSha256(target) !== expected) problems.push(`canonical catalog integrity mismatch: ${relativePath}`);
    } catch (error) {
      problems.push(`canonical catalog is not valid JSON: ${relativePath}: ${error.message}`);
    }
  }
  return { checked: true, valid: problems.length === 0, catalogCount: Object.keys(CANONICAL_CATALOG_SHA256).length, problems };
}

async function main() {
  const args = parseRepoFileArgs(process.argv.slice(2));
  if (args.help) return console.log("Usage: node scripts/catalog-integrity-gate.mjs --repo .");
  gateResult("canonical control catalogs", validateCatalogIntegrity(ASSET_ROOT));
}

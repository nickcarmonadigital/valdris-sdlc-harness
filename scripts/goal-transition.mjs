...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === "--repo") args.repo = argv[++index];
      ...
      args.checkpointStatus = argv[++index];
...
function main() {
  ...
  if (args.help)
    return console.log(
      "Usage: node scripts/goal-transition.mjs --repo . --expected-revision N [--checkpoint id --checkpoint-status progress --summary text] [--condition id --condition-status passed --evidence-file evidence.json] [--goal-status completed]",
    );
  ...
  try {
    ...
    writeFileSync(
      lock,
      `${JSON.stringify({ pid: process.pid, acquiredAt: new Date().toISOString(), expectedRevision: args.expectedRevision })}\n`,
    );
  } catch (error) {
    if (lock !== undefined) closeSync(lock);
    throw new Error(
      `goal transition locked by another writer: ${error.message}`,
    );
  }
  try {
    const goal = JSON.parse(readFileSync(target, "utf8"));
    if (goal.revision !== args.expectedRevision)
      throw new Error(
        `goal revision conflict: expected ${args.expectedRevision}, current ${goal.revision}`,
      );
    const now = new Date().toISOString();

    if (args.checkpoint) {
      if (
        !new Set(["started", "progress", "passed", "failed", "blocked"]).has(
          args.checkpointStatus,
        )
      )
        throw new Error("--checkpoint-status is invalid");
      if (!args.summary?.trim())
        throw new Error("--summary is required for a checkpoint transition");
      const checkpoint = goal.checkpoints.find(
        (item) => item.id === args.checkpoint,
      );
      if (checkpoint)
    ...
    if (args.condition) {
      if (
        !new Set(["pending", "passed", "failed", "blocked"]).has(
          args.conditionStatus,
        )
      )
        throw new Error("--condition-status is invalid");
      const condition = goal.stoppingConditions.find(
        (item) => item.id === args.condition,
      );
      if (!condition)
        throw new Error(`unknown stopping condition: ${args.condition}`);
      condition.status = args.conditionStatus;
      if (args.conditionStatus === "passed") {
        if (!args.evidenceFile)
          throw new Error(
            "--evidence-file is required when passing a stopping condition",
          );
        const evidence = JSON.parse(
          readFileSync(path.resolve(args.evidenceFile), "utf8"),
        );
        if (!Array.isArray(evidence) || evidence.length === 0)
          throw new Error("evidence file must contain a non-empty JSON array");
        condition.evidence = evidence;
      } else delete condition.evidence;
    }

    if (args.goalStatus) goal.status = args.goalStatus;
    if (!args.checkpoint && !args.condition && !args.goalStatus)
    ...
    if (!existingFileWithinRepo(repoRoot, classificationPath))
      throw new Error(
        "goal transition requires run/workload-classification.json",
      );
    const routePath = path.join(repoRoot, "run", "route.json");
    if (!existingFileWithinRepo(repoRoot, routePath))
      throw new Error("goal transition requires run/route.json");
    const validation = validateGoalDocument(goal, {
      repoRoot,
      requireComplete: goal.status === "completed",
    ...
    if (!validation.valid)
      throw new Error(
        `goal transition rejected: ${validation.problems.join("; ")}`,
      );

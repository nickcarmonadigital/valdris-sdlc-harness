...
function resolveContextFileReference(repoRoot, relativePath, label, problems) {
  ...
  if (isSecretLike(relativePath)) problems.push(`${label} is secret-like and must not be loaded: ${relativePath}`);
  ...
  for (const component of targetRelative.split(path.sep).filter(Boolean)) {
    ...
    if (lstatSync(cursor).isSymbolicLink()) {
      problems.push(`${label} contains a symbolic-link component and must be a real non-symlink file inside the repository`);
      break;
    }
  }
  ...
  const canonicalRelativePath = path.relative(realpathSync(repoRoot), realpathSync(target)).split(path.sep).join("/");
  if (isSecretLike(canonicalRelativePath) && canonicalRelativePath !== relativePath.replaceAll("\\", "/")) {
    problems.push(`${label} canonical target is secret-like and must not be loaded: ${canonicalRelativePath}`);
  }
...
function validateQualityArtifact(entry, label, repoRoot, problems) {
  ...
  if (!Number.isInteger(entry.caseCount) || entry.caseCount <= 0) problems.push(`${label}.caseCount must be a positive integer`);
  ...
  const target = resolveContextFileReference(repoRoot, relativePath, `${label}.path`, problems);
  if (!target) return;
  if (/^[a-f0-9]{64}$/i.test(nonEmpty(entry.sha256)) && sha256File(target) !== entry.sha256.toLowerCase()) problems.push(`${label}.sha256 does not match ${relativePath}`);
}

export function validateContextQualityContract(contract, options = {}) {
  const repoRoot = path.resolve(options.repoRoot || process.cwd());
  const problems = [];
  if (!contract || typeof contract !== "object" || Array.isArray(contract)) return { valid: false, problems: ["context manifest contextQuality is required when loadedFiles are present"] };
  if (contract.schema !== CONTEXT_QUALITY_SCHEMA) problems.push(`context manifest contextQuality.schema must be ${CONTEXT_QUALITY_SCHEMA}`);
  if (!nonEmpty(contract.suiteId)) problems.push("context manifest contextQuality.suiteId is required");
  if (!CONTEXT_BASELINE_MODES.has(contract.baselineMode)) problems.push("context manifest contextQuality.baselineMode must be no-context or limited-context");
  validateQualityArtifact(contract.caseSet, "context manifest contextQuality.caseSet", repoRoot, problems);
  validateQualityArtifact(contract.answerKey, "context manifest contextQuality.answerKey", repoRoot, problems);
  if (contract.caseSet?.path && contract.caseSet.path === contract.answerKey?.path) problems.push("context manifest contextQuality caseSet and answerKey must use distinct paths");
  if (Number.isInteger(contract.caseSet?.caseCount) && Number.isInteger(contract.answerKey?.caseCount) && contract.caseSet.caseCount !== contract.answerKey.caseCount) problems.push("context manifest contextQuality answerKey.caseCount must match caseSet.caseCount");
  if (!contract.metric || typeof contract.metric !== "object" || Array.isArray(contract.metric)) problems.push("context manifest contextQuality.metric must be an object");
  else {
    if (!nonEmpty(contract.metric.id)) problems.push("context manifest contextQuality.metric.id is required");
    if (!METRIC_DIRECTIONS.has(contract.metric.direction)) problems.push("context manifest contextQuality.metric.direction must be higher-is-better or lower-is-better");
    if (!Number.isFinite(contract.metric.minDelta) || contract.metric.minDelta <= 0) problems.push("context manifest contextQuality.metric.minDelta must be greater than 0");
    if (!Number.isFinite(contract.metric.candidateThreshold)) problems.push("context manifest contextQuality.metric.candidateThreshold must be numeric");
  }
  return { valid: problems.length === 0, schema: contract.schema, suiteId: contract.suiteId, problems };
}
...
export function validateContextManifestDocument(document, options = {}) {
  ...
  const contextQuality = validateContextQualityContract(document.contextQuality, { repoRoot });

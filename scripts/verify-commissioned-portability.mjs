...
const PORTABILITY_WALL_CLOCK_MS =
  configuredPortabilityTimeout === undefined
    ? 600_000
    : Number(configuredPortabilityTimeout);
if (
  !Number.isFinite(PORTABILITY_WALL_CLOCK_MS) ||
  PORTABILITY_WALL_CLOCK_MS <= 0
) {
  throw new Error(
...
function walkRegularFiles(root, current = root, files = []) {
  for (const entry of readdirSync(current, { withFileTypes: true })) {
    ...
    assert(
      ...
      `artifact bundle fixture must not traverse a symlink: ${target}`,
    );
    if (stats.isDirectory()) walkRegularFiles(root, target, files);
    else {
      assert(
        stats.isFile(),
        `artifact bundle fixture contains a non-regular file: ${target}`,
      );
...
function isTracked(repo, relativePath) {
  return (
    run("git", ["ls-files", "--error-unmatch", "--", relativePath], repo)
      .status === 0
  );
}
...
function assertOutputOmits(result, forbidden, label) {
  ...
  assert(
    ...
    `${label} leaked untrusted input in diagnostics`,
  );
}
...
async function postJson(
  ...
  assert(
    ...
    `bridge ${pathname} expected ${expectedStatus}, got ${response.status}: ${JSON.stringify(parsed)}`,
  );
  return parsed;
}
...
function approvalEvidence(goal, conditionId, approvedAt, expiresAt) {
  return {
    ...
    producer: {
      name: "Commissioning Portability Owner",
      version: "1.0.0",
      kind: "human",
    },
    actorType: "human",
...
function verifySubdirectoryLoaderBinding(tempRoot, commissionScript) {
  ...
  git(worktree, [
    "config",
    "user.email",
    "commissioning-subdirectory@example.com",
  ]);
  ...
  expectOk(
    runNode(
      commissionScript,
      ["--repo", target, "--project-name", "Subdirectory Target", "--yes"],
      HARNESS_ROOT,
    ),
  ...
  process.env.UASH_REVIEW_TRUST_SHA256 = reviewTrustSha256(
    path.join(pack, "controls", "review-trust.v1.json"),
  );
  git(target, ["add", ".valdris-harness"]);
  git(target, [
    "commit",
    "-m",
    "test: commit only the nested Valdris runtime pack",
  ]);
  ...
  expectFailure(
    runNode(
      path.join(pack, "scripts", "run-packet-gate.mjs"),
      ["--repo", ".", "--print-runtime-binding", "--commit", packOnlyCommit],
      target,
    ),
  ...
  git(target, [
    "commit",
    "-m",
    "chore: bind target-root Valdris discovery loaders",
  ]);
  const commit = git(target, ["rev-parse", "HEAD"]);
  const result = expectOk(
    runNode(
      path.join(pack, "scripts", "run-packet-gate.mjs"),
      ["--repo", ".", "--print-runtime-binding", "--commit", commit],
      target,
    ),
    "subdirectory validation-runtime binding",
  );
  const binding = JSON.parse(result.stdout);
  assert(
    binding.source?.targetPath === "apps/[ios]-game",
    "runtime binding must use the literal Git-native target prefix inside the containing worktree",
  );
  assert(
    binding.source?.runtimePath === "apps/[ios]-game/.valdris-harness",
    "runtime binding must use the literal Git-native nested runtime prefix inside the containing worktree",
  );
  const loaderBindings = binding.files.filter(
    ({ kind }) => kind === "target-root-discovery-loader",
  );
  assert(
    loaderBindings.length === 2,
    "subdirectory runtime binding must include both target-root discovery loaders",
  );
  for (const { path: fileName, sha256: digest } of loaderBindings) {
    const committed = expectOk(
      run("git", ["show", `HEAD:apps/[ios]-game/${fileName}`], target),
      `read special-path committed ${fileName} blob`,
    ).stdout;
    assert(
      digest === sha256(committed),
      `subdirectory loader binding must use the stable committed ${fileName} blob`,
    );
  }
}
...
async function main() {
  ...
  if (verifierArgs.some((arg) => arg !== "--acceptance-only"))
    throw new Error(
      "Usage: node scripts/verify-commissioned-portability.mjs [--acceptance-only]",
    );
  const acceptanceOnly = verifierArgs.includes("--acceptance-only");
  const tempRoot = realpathSync(
    mkdtempSync(path.join(tmpdir(), "valdris-commissioned-portability-")),
  );
  ...
  try {
    ...
    writeFileSync(
      path.join(target, "Package.swift"),
      "// swift-tools-version: 6.0\n",
      "utf8",
    );
    writeFileSync(
      path.join(target, "ValdrisGame.xcodeproj", "project.pbxproj"),
      "// !$*UTF8*$!\n",
      "utf8",
    );
    writeFileSync(
      path.join(target, "Assets.xcassets", "AppIcon.appiconset", "icon.png"),
    ...
    git(productWorktree, [
      "commit",
      "-m",
      "test: initialize containing iOS-like product worktree",
    ]);
    ...
    expectFailure(
      runNode(
        ...
        [
          ...
          "Valdris iOS Game",
          "--out",
          path.join(tempRoot, "external-pack"),
          "--yes",
        ],
        HARNESS_ROOT,
      ),
    ...
    expectOk(
      runNode(
        commissionScript,
        ["--repo", target, "--project-name", "Valdris iOS Game", "--yes"],
        HARNESS_ROOT,
      ),
    ...
    assert(
      ...
      "adapter must define target-root-relative command semantics",
    );
    assert(
      adapter.installation?.packRoot === ".valdris-harness",
      "adapter must bind the canonical nested pack path",
    );
    assert(
      ...
      "adapter must require a committed same-worktree pack and target-root discovery loaders",
    );
    assert(
      adapter.reviewTrust?.path ===
        ".valdris-harness/controls/review-trust.v1.json",
      "review trust must bind the target-relative nested path",
    );
    assert(
      readFileSync(path.join(target, "AGENTS.md"), "utf8").includes(
        ".valdris-harness/AGENTS.md",
      ),
      "target-root AGENTS.md discovery loader missing",
    );
    assert(
      readFileSync(path.join(target, "CLAUDE.md"), "utf8").includes(
        "@.valdris-harness/CLAUDE.md",
      ),
      "target-root CLAUDE.md discovery loader missing",
    );
    const structuralWorkflow = readFileSync(
      path.join(pack, ".github", "workflows", "valdris-assurance.yml"),
      "utf8",
    );
    const acceptanceWorkflow = readFileSync(
      path.join(pack, ".github", "workflows", "valdris-run-acceptance.yml"),
      "utf8",
    );
    assert(
    ...
    assert(
      ...
      "generated acceptance workflow interpolates an untrusted expression into a shell command",
    );
    for (const relativePath of [
      ...
      assert(
        !text.includes("node scripts/"),
        `${relativePath} contains an unsupported root-runtime command`,
      );
      assert(
        text.includes(".valdris-harness"),
        `${relativePath} does not identify the canonical nested runtime`,
      );
    }
    ...
    const commissionedReviewTrustSha256 = reviewTrustSha256(
      path.join(pack, "controls", "review-trust.v1.json"),
    );
    ...
    git(target, [
      "commit",
      "-m",
      "chore: commission Valdris runtime pack and discovery loaders",
    ]);

    const privacyScript = path.join(pack, "scripts", "privacy-gate.mjs");
    expectOk(
      runNode(privacyScript, ["--repo", ".valdris-harness"], target),
      "pack-scoped clean-room privacy gate",
    );
    expectFailure(
      runNode(privacyScript, ["--repo", "."], target),
      "arbitrary full product privacy scan",
      "unapproved-binary",
    );

    const graphScan = path.join(pack, "scripts", "code-intelligence-scan.mjs");
    expectOk(
      runNode(graphScan, ["--repo", ".", "--provider", "local"], target),
      "generated code-intelligence scan",
    );
    ...
    assert(
      evidencePrivacyReport.scope?.mode === "include" &&
        evidencePrivacyReport.scope.paths.includes("graph"),
      "evidence privacy report must disclose its bounded scope",
    );
    git(target, ["add", "graph", "design"]);
    git(target, [
      "commit",
      "-m",
      "chore: bind generated code-intelligence evidence",
    ]);
    const commit = git(target, ["rev-parse", "HEAD"]);
    ...
    expectOk(
      runNode(
        ...
        [
          ...
          environment,
          "--actor",
          "Commissioning Portability Owner",
          "--request",
          "Copy edit the iOS game README wording only.",
        ],
        target,
      ),
      "generated route request",
    );
    for (const gate of [
      "intake-gate.mjs",
      ...
      expectOk(
        runNode(path.join(pack, "scripts", gate), ["--repo", "."], target),
        `generated ${gate}`,
      );
    }
    ...
    assert(
      goal.commit === commit,
      "generated goal must bind the current target commit",
    );
    ...
    goal.checkpoints = goal.checkpoints.map((checkpoint) => ({
      ...
      status: "passed",
      summary:
        "Nested commissioned runtime completed the focused portability route.",
    }));
    ...
    writeFileSync(
      tracePath,
      `${JSON.stringify({ runId, event: "commissioning-portability-complete" })}\n`,
      "utf8",
    );
    ...
    expectOk(
      runNode(
        path.join(pack, "scripts", "goal-gate.mjs"),
        ["--repo", "."],
        target,
      ),
      "generated completed-goal gate",
    );
    expectOk(
      runNode(
        path.join(pack, "scripts", "trajectory-gate.mjs"),
        ["--repo", "."],
        target,
      ),
      "generated trajectory gate",
    );
    ...
    writeFileSync(
      path.join(target, "handoff", "final.md"),
      "# Commissioned portability handoff\n\nAll genuine v0.8 gates passed.\n",
      "utf8",
    );
    const classification = readJson(
      path.join(target, "run", "workload-classification.json"),
    );
    const route = readJson(path.join(target, "run", "route.json"));
    const classificationSha256 = sha256(
      readFileSync(path.join(target, "run", "workload-classification.json")),
    );
    const skippedBinding = {
      effectiveTier: classification.effectiveTier,
      workloadClassificationSha256: classificationSha256,
    };
    writeJson(path.join(target, "evals", "context-quality-cases.json"), {
    ...
    writeJson(path.join(target, "evals", "context-quality-answer-key.json"), {
    ...
    const contextCaseSet = {
      id: "commissioned-context-cases",
      version: "v1",
      path: "evals/context-quality-cases.json",
      sha256: sha256(
        readFileSync(path.join(target, "evals", "context-quality-cases.json")),
      ),
      caseCount: 2,
    };
    const contextAnswerKey = {
      id: "commissioned-context-answer-key",
      version: "v1",
      path: "evals/context-quality-answer-key.json",
      sha256: sha256(
        readFileSync(
          path.join(target, "evals", "context-quality-answer-key.json"),
        ),
      ),
      caseCount: 2,
    };
    ...
    writeJson(path.join(target, "context", "manifest.json"), {
      ...
      loadedFiles: [
        {
          path: "Package.swift",
          sha256: sha256(readFileSync(path.join(target, "Package.swift"))),
    ...
    const contextManifestSha256 = sha256(
      readFileSync(path.join(target, "context", "manifest.json")),
    );
    ...
    const contextConfigDigest = sha256(
      "commissioned-context-verifier-config-v1",
    );
    ...
    writeJson(path.join(target, "domain", "assurance.json"), {
      ...
      packs: [],
      skipReason:
        "The validated T0 documentation route has no product domain pack.",
    });
      ...
      suites: [
        {
          ...
          resultDigest: sha256(
            readFileSync(
              path.join(target, "proof", "context-candidate-results.json"),
            ),
          ),
          threshold: 0.9,
          value: 1,
          ...
          contextComparison: {
            ...
            baseline: {
              ...
              resultDigest: sha256(
                readFileSync(
                  path.join(target, "proof", "context-baseline-results.json"),
                ),
              ),
            },
            candidate: {
    ...
    expectOk(
      runNode(
        path.join(pack, "scripts", "enterprise-ai-gate-all.mjs"),
        ["--repo", "."],
        target,
      ),
      "genuine commissioned aggregate finish-line gate",
    );
    ...
    expectOk(
      ...
      "generated portable proof runner",
    );

    const runtimeBindingResult = expectOk(
      runNode(
        path.join(pack, "scripts", "run-packet-gate.mjs"),
        ["--repo", ".", "--print-runtime-binding", "--commit", commit],
        target,
      ),
      "nested validation-runtime binding",
    );
    const runtimeBinding = JSON.parse(runtimeBindingResult.stdout);
    assert(
      runtimeBinding.source?.targetPath === "apps/ios-game",
      "final runtime binding must identify the nested target path",
    );
    assert(
      runtimeBinding.source?.runtimePath === "apps/ios-game/.valdris-harness",
      "final runtime binding must identify the commissioned pack inside the nested target",
    );
    const loaderBindings = runtimeBinding.files.filter(
      ({ kind }) => kind === "target-root-discovery-loader",
    );
    assert(
      loaderBindings.length === 2,
      "runtime binding must include both target-root discovery loaders",
    );
    for (const fileName of ["AGENTS.md", "CLAUDE.md"]) {
      const loaderBinding = loaderBindings.find(
        ({ path: bindingPath }) => bindingPath === fileName,
      );
      const committedLoader = expectOk(
        run("git", ["show", `HEAD:apps/ios-game/${fileName}`], target),
        `read committed ${fileName} blob`,
      ).stdout;
      assert(
        loaderBinding?.sha256 === sha256(committedLoader),
        `runtime binding must digest-bind the committed target-root ${fileName} blob`,
      );
    }
    const runtimeWithoutLoaders = runtimeBinding.files.filter(
      ({ kind }) => kind !== "target-root-discovery-loader",
    );
    assert(
      runtimeBinding.setSha256 !==
        sha256(
          canonicalJson({
            source: runtimeBinding.source,
            files: runtimeWithoutLoaders,
          }),
        ),
      "validation runtime setSha256 must depend on the target-root discovery loaders",
    );

    const evidenceBundleResult = expectOk(
      runNode(
      ...
      "generated review evidence-bundle projection",
    );
    const evidenceBundle = JSON.parse(evidenceBundleResult.stdout);
    assert(
      /^[a-f0-9]{64}$/.test(evidenceBundle.evidenceBundleSha256 || ""),
      "review evidence bundle must have a SHA-256 binding",
    );
    ...
    const review = {
      ...
      roleProvenance: {
        ...
        scout: {
          actorId: "EXAMPLE-COMMISSION-SCOUT",
          actorType: "agent",
          sessionId: "EXAMPLE-COMMISSION-SCOUT-SESSION",
          executionId: "EXAMPLE-COMMISSION-SCOUT-EXECUTION",
          evidence: {
            kind: "artifact",
            path: "run/route.json",
            sha256: sha256(
              readFileSync(path.join(target, "run", "route.json")),
            ),
          },
        },
        implementer: {
        ...
        verifier: {
          actorId: "EXAMPLE-COMMISSION-VERIFIER",
          actorType: "agent",
          sessionId: "EXAMPLE-COMMISSION-VERIFY-SESSION",
          executionId: "EXAMPLE-COMMISSION-VERIFY-EXECUTION",
          evidence: {
            kind: "artifact",
            path: "proof/portable.json",
            sha256: proofDigest,
      ...
      decision: {
        status: "accepted",
        summary:
          "Nested commissioned proof satisfies the portability contract.",
      },
      findings: [],
      blockers: [],
    };
    ...
    expectOk(
      runNode(
        ...
        ["--repo", ".", "--file", "review/review.json"],
        target,
      ),
      "pack-aware signed review gate",
    );
    ...
    writeJson(
      path.join(target, "review", "review.json"),
      missingVerifierReview,
    );
    expectFailure(
      runNode(
        path.join(pack, "scripts", "review-gate.mjs"),
      ...
      "commissioned review without verifier provenance",
      "roleProvenance.verifier is required",
    );

    const collapsedReviewer = structuredClone(review);
    collapsedReviewer.roleProvenance.independentReviewer.actorId =
      collapsedReviewer.roleProvenance.implementer.actorId;
    ...
    expectFailure(
      ...
      "commissioned review with collapsed implementer/reviewer identity",
      "actorId is reused",
    );
    ...
    expectOk(
      ...
      "restored four-role commissioned review gate",
    );
    ...
    expectOk(
      runNode(
        ...
        [
          ...
          "trajectory=trajectory/trajectory.json",
          "--output",
          path.join(target, "run", "packet.json"),
        ],
        target,
      ),
      "generated run-packet creation",
    );
    expectOk(
      runNode(
        path.join(pack, "scripts", "run-packet-gate.mjs"),
        ["--repo", ".", "--file", "run/packet.json"],
        target,
      ),
      "generated run-packet validation",
    );
    const packetDocument = readJson(path.join(target, "run", "packet.json"));
    assert(
      !packetDocument.artifactInventory.some(
        ({ path: artifactPath }) =>
          artifactPath === "review/review.json" ||
          artifactPath === "run/packet.json",
      ),
      "absolute in-repository CLI paths bypassed canonical run-packet inventory exclusions",
    );
    assert(
      packetDocument.validationRuntime?.reviewTrustSha256 ===
        commissionedReviewTrustSha256,
      "final packet must bind the operator-held review trust-store pin",
    );
    assert(
      packetDocument.validationRuntime?.setSha256 === runtimeBinding.setSha256,
      "final packet must carry the loader-bound validation runtime digest",
    );
    assert(
      packetDocument.bindings?.validationRuntimeSha256 ===
        runtimeBinding.setSha256,
      "final packet envelope must bind the loader-bound validation runtime digest",
    );
    const expectedRoleProvenanceSha256 = sha256(
      canonicalJson(review.roleProvenance),
    );
    assert(
      packetDocument.roleProvenanceSha256 === expectedRoleProvenanceSha256,
      "final packet must expose the signed four-role provenance digest",
    );
    assert(
      packetDocument.bindings?.roleProvenanceSha256 ===
        expectedRoleProvenanceSha256,
      "final packet envelope must bind the signed four-role provenance digest",
    );

    const acceptanceBundle = path.join(tempRoot, "acceptance-artifact-bundle");
    const bundledFiles = createAcceptanceArtifactBundle(
      target,
      acceptanceBundle,
      commit,
    );
    assert(
      bundledFiles.some(
        ({ path: artifactPath }) => artifactPath === "run/packet.json",
      ),
      "acceptance bundle fixture must contain the final run packet",
    );
    assert(
      !bundledFiles.some(({ path: artifactPath }) =>
        artifactPath.startsWith(".valdris-harness/"),
      ),
      "acceptance bundle fixture must never contain commissioned validator files",
    );
    const acceptanceWorktree = path.join(
      tempRoot,
      "acceptance-source-worktree",
    );
    git(productWorktree, [
      "-c",
      "core.autocrlf=false",
    ...
    const malformedManifestBundle = path.join(
      tempRoot,
      "malformed-manifest-bundle",
    );
    mkdirSync(malformedManifestBundle, { recursive: true });
    writeFileSync(
      path.join(malformedManifestBundle, "valdris-run-artifacts.json"),
      `{\"untrusted\":\"${malformedManifestSecret}\"`,
      "utf8",
    );
    const malformedManifest = expectFailure(
      runNode(
        acceptanceScript,
        [
          "--repo",
          ".",
          "--bundle",
          malformedManifestBundle,
          "--source-commit",
          commit,
        ],
        acceptanceTarget,
      ),
      "run acceptance malformed manifest",
      "valdris-run-artifacts.json must be valid JSON",
    );
    assertOutputOmits(
      malformedManifest,
      malformedManifestSecret,
      "malformed manifest diagnostics",
    );

    const malformedPacketSecret = "MALFORMED-PACKET-SECRET";
    const malformedPacketBundle = path.join(
      tempRoot,
      "malformed-packet-bundle",
    );
    createFixtureArtifactBundle(malformedPacketBundle, commit, {
      "run/packet.json": `{\"untrusted\":\"${malformedPacketSecret}\"`,
    });
    const malformedPacket = expectFailure(
      runNode(
        acceptanceScript,
        [
          "--repo",
          ".",
          "--bundle",
          malformedPacketBundle,
          "--source-commit",
          commit,
        ],
        acceptanceTarget,
      ),
      "run acceptance malformed packet",
      "bundled run/packet.json must be valid JSON",
    );
    assertOutputOmits(
      malformedPacket,
      malformedPacketSecret,
    ...
    createFixtureArtifactBundle(deepBundle, commit, {
      "run/packet.json": readFileSync(
        path.join(acceptanceBundle, "run", "packet.json"),
      ),
    });
    const deepRelativePath = `run/${Array.from({ length: 17 }, (_, index) => `nested-${index}`).join("/")}/extra.json`;
    writeJson(path.join(deepBundle, ...deepRelativePath.split("/")), {
      untrusted: true,
    });
    expectFailure(
      runNode(
      ...
      "run acceptance deeply nested bundle",
    ...
    expectFailure(
      ...
      "run acceptance source checkout mismatch",
    ...
    const validatorOverwriteBundle = path.join(
      tempRoot,
      "validator-overwrite-bundle",
    );
    createFixtureArtifactBundle(validatorOverwriteBundle, commit, {
      ".valdris-harness/scripts/run-acceptance.mjs":
        "attacker-controlled-validator\n",
      "run/packet.json": readFileSync(
        path.join(acceptanceBundle, "run", "packet.json"),
      ),
    });
    expectFailure(
      runNode(
        ...
        [
          ...
          ".",
          "--bundle",
          validatorOverwriteBundle,
          "--source-commit",
          commit,
        ],
        acceptanceTarget,
      ),
      "run acceptance validator overwrite",
      "outside the evidence namespaces",
    );
    assert(
      sha256(readFileSync(validatorPath)) === validatorDigestBefore,
      "validator overwrite rejection changed the commissioned acceptance CLI",
    );

    const sourceOverwriteBundle = path.join(
      tempRoot,
      "source-overwrite-bundle",
    );
    createFixtureArtifactBundle(sourceOverwriteBundle, commit, {
      "design/anchors.json": readFileSync(
        path.join(target, "design", "anchors.json"),
      ),
      "run/packet.json": readFileSync(
        path.join(acceptanceBundle, "run", "packet.json"),
      ),
    ...
    expectFailure(
      runNode(
        ...
        [
          ...
          ".",
          "--bundle",
          sourceOverwriteBundle,
          "--source-commit",
          commit,
        ],
        acceptanceTarget,
      ),
      "run acceptance source overwrite",
      "refuses to overwrite an existing source or validator path",
    );
    assert(
      !existsSync(path.join(acceptanceTarget, "run", "packet.json")),
      "failed preflight partially hydrated the run packet",
    );
    ...
    const rejectedReviewPath = path.join(
      rejectedGateBundle,
      "review",
      "review.json",
    );
    const rejectedReview = readJson(rejectedReviewPath);
    rejectedReview.attestation.signature = Buffer.from(
      "invalid-staged-review",
      "utf8",
    ).toString("base64");
    writeJson(rejectedReviewPath, rejectedReview);
    const rejectedManifestPath = path.join(
      rejectedGateBundle,
      "valdris-run-artifacts.json",
    );
    const rejectedManifest = readJson(rejectedManifestPath);
    const rejectedReviewEntry = rejectedManifest.files.find(
      ({ path: artifactPath }) => artifactPath === "review/review.json",
    );
    ...
    expectFailure(
      ...
      "run acceptance staged gate rejection",
      "review attestation signature is invalid",
    );
    assert(
      !existsSync(path.join(acceptanceTarget, "run", "packet.json")),
      "failed staged validation hydrated the run packet into the source checkout",
    );
    assert(
      !existsSync(path.join(acceptanceTarget, "review", "review.json")),
      "failed staged validation hydrated the rejected review into the source checkout",
    );

    const workflowAcceptanceEnv = { ...process.env };
    const inheritedPathKey = Object.keys(workflowAcceptanceEnv).find(
      (name) => name.toLowerCase() === "path",
    );
    assert(
      inheritedPathKey,
      "commissioned acceptance verifier requires a process path variable",
    );
    const inheritedPath = workflowAcceptanceEnv[inheritedPathKey];
    const mixedCasePathKey = process.platform === "win32" ? "Path" : "pAtH";
    if (process.platform === "win32")
      for (const name of Object.keys(workflowAcceptanceEnv))
        if (name.toLowerCase() === "path") delete workflowAcceptanceEnv[name];
    ...
    const acceptanceResult = expectOk(
      runNode(acceptanceScript, ["--repo", "."], acceptanceTarget, {
        env: workflowAcceptanceEnv,
      }),
      "generated detached-source run acceptance",
    );
    const acceptanceReport = JSON.parse(acceptanceResult.stdout);
    assert(
      acceptanceReport.ok === true && acceptanceReport.sourceCommit === commit,
      "run acceptance must report the exact accepted source commit",
    );
    assert(
      acceptanceReport.hydratedFiles === bundledFiles.length,
      "run acceptance must report every safely hydrated artifact",
    );
    assert(
      acceptanceReport.pathEnvironmentKeys?.includes(mixedCasePathKey),
      "run acceptance must preserve a mixed-case Path key for child validator tool lookup",
    );
    ...
    const bridge = trackedChild(
      spawn(
        process.execPath,
        [path.join(HARNESS_ROOT, "scripts", "claude-code-bridge.mjs")],
        {
          cwd: HARNESS_ROOT,
          env: {
            ...process.env,
            ...
            [["UASH", "HUMAN", "APPROVAL", "TOKEN"].join("_")]:
              bridgeApprovalCredential,
          },
          stdio: "ignore",
        },
      ),
    ...
    try {
      ...
      const created = await postJson(bridgePort, "/runs", {
        id: runId,
        artifactRoot: target,
      });
      assert(
        created.reviewTrustSha256 === commissionedReviewTrustSha256,
        "bridge did not seal the operator-held review trust-store pin into the run",
      );
      assert(
        created.commissionedRuntime?.runtimeSha256 ===
          runtimeBinding.runtimeSha256,
        "bridge did not persist the genuine commissioned runtime and loader identity",
      );
      assert(
        created.adapterPolicy?.enterpriseFinishLineRequired === true &&
          created.adapterPolicy?.portableFinishLineRequired === true,
        "bridge did not load mandatory v0.8 finish-line policy",
      );
      ...
      await postJson(
        bridgePort,
        `/runs/${runId}/events`,
        event("approval.requested", "route", "route approval required", {
      ...
      await postJson(
        bridgePort,
        `/runs/${runId}/events`,
        event("approval.granted", "route", "route approved", {
          actor: "human",
          artifact: "run/route.json",
          approvalOwner: "Commissioning Portability Owner",
          approvalScope: "route",
        ...
        200,
        {
          [["x", "uash", "human", "token"].join("-")]: bridgeApprovalCredential,
        },
      );
      for (const nodeId of adapter.runtime.requiredNodes.filter(
        (node) => !["prove", "handoff"].includes(node),
      )) {
        await postJson(
          bridgePort,
          `/runs/${runId}/events`,
          event(
            "node.skipped",
            nodeId,
            `${nodeId} is not applicable to the validated T0 documentation route`,
            {
              status: "skipped",
              skipReason:
                "The validated T0 documentation route marks this runtime node not applicable; required finish-line artifacts remain independently gated.",
            },
          ),
        );
      }
      await postJson(
        bridgePort,
        `/runs/${runId}/events`,
        event("artifact.written", "prove", "bridge proof attached", {
      ...
      await postJson(
        bridgePort,
        `/runs/${runId}/events`,
        event("artifact.written", "handoff", "bridge handoff attached", {
      ...
      invalidReview.attestation.signature = Buffer.from(
        "invalid-review-signature",
        "utf8",
      ).toString("base64");
      writeJson(reviewPath, invalidReview);
      const invalidReviewBlocked = await postJson(
        bridgePort,
        `/runs/${runId}/events`,
        event("run.completed", "handoff", "reject invalid independent review", {
          artifact: "handoff/final.md",
        }),
        409,
      );
      assert(
        (invalidReviewBlocked.problems || []).some((problem) =>
          String(problem).includes(
            "v0.8 independent review finish line failed",
          ),
        ),
        "bridge did not invoke the genuine nested independent-review gate",
      );
      ...
      await postJson(
        ...
        event(
          "artifact.written",
          "prove",
          "bridge proof revalidated after review rejection",
          { artifact: "proof/proof.json" },
        ),
      );
      ...
      const invalidPacketBlocked = await postJson(
        bridgePort,
        `/runs/${runId}/events`,
        event("run.completed", "handoff", "reject invalid final packet", {
          artifact: "handoff/final.md",
        }),
        409,
      );
      assert(
        (invalidPacketBlocked.problems || []).some((problem) =>
          String(problem).includes("v0.8 run packet finish line failed"),
        ),
        "bridge did not invoke the genuine nested run-packet gate",
      );
      ...
      await postJson(
        ...
        event(
          "artifact.written",
          "prove",
          "bridge proof revalidated after packet rejection",
          { artifact: "proof/proof.json" },
        ),
      );

      writeJson(path.join(target, "rca", "rca.json"), {
        schema: "valdris.rca.v1",
        status: "invalid-fixture",
      });
      const invalidRcaBlocked = await postJson(
        bridgePort,
        `/runs/${runId}/events`,
        event("run.completed", "handoff", "reject invalid conditional RCA", {
          artifact: "handoff/final.md",
        }),
        409,
      );
      assert(
        (invalidRcaBlocked.problems || []).some((problem) =>
          String(problem).includes("v0.8 RCA finish line failed"),
        ),
        "bridge did not invoke the genuine nested RCA gate when an RCA artifact existed",
      );
      ...
      await postJson(
        ...
        event(
          "artifact.written",
          "prove",
          "bridge proof revalidated after RCA rejection",
          { artifact: "proof/proof.json" },
        ),
      );
      ...
      const omittedEvalPrivacyBlocked = await postJson(
        bridgePort,
        `/runs/${runId}/events`,
        event(
          "run.completed",
          "handoff",
          "reject private contact in packet-bound eval evidence",
          { artifact: "handoff/final.md" },
        ),
        409,
      );
      assert(
        (omittedEvalPrivacyBlocked.problems || []).some((problem) =>
          String(problem).includes("run-artifact privacy finish line failed"),
        ),
        "packet-bound eval evidence was omitted from the commissioned privacy closure",
      );
      assert(
        !JSON.stringify(omittedEvalPrivacyBlocked).includes(privateContact),
        "packet-closure privacy rejection disclosed private contact data",
      );
      ...
      await postJson(
        ...
        event(
          "artifact.written",
          "prove",
          "bridge proof revalidated before referenced-evidence privacy test",
          { artifact: "proof/proof.json" },
        ),
      );
      ...
      const referencedTracePrivacyBlocked = await postJson(
        bridgePort,
        `/runs/${runId}/events`,
        event(
          "run.completed",
          "handoff",
          "reject private contact in digest-valid referenced trace evidence",
          { artifact: "handoff/final.md" },
        ),
        409,
      );
      assert(
        (referencedTracePrivacyBlocked.problems || []).some((problem) =>
          String(problem).includes("run-artifact privacy finish line failed"),
        ),
        "digest-valid trajectory trace evidence was omitted from the commissioned privacy closure",
      );
      assert(
        !JSON.stringify(referencedTracePrivacyBlocked).includes(
          referencedPrivateContact,
        ),
        "referenced-trace privacy rejection disclosed private contact data",
      );
      ...
      await postJson(
        ...
        event(
          "artifact.written",
          "prove",
          "bridge proof revalidated after packet-closure privacy rejection",
          { artifact: "proof/proof.json" },
        ),
      );
      ...
      const assuranceEvidencePath = path.join(
        target,
        "proof",
        "assurance-private-evidence.txt",
      );
      const assurancePrivateContact = `${"assurance-owner"}@${"private"}.${"example"}`;
      writeFileSync(
        assuranceEvidencePath,
        `contact=${assurancePrivateContact}\n`,
        "utf8",
      );
      ...
      const referencedAssurancePrivacyBlocked = await postJson(
        bridgePort,
        `/runs/${runId}/events`,
        event(
          "run.completed",
          "handoff",
          "reject private contact in digest-valid assurance evidence",
          { artifact: "handoff/final.md" },
        ),
        409,
      );
      assert(
        (referencedAssurancePrivacyBlocked.problems || []).some((problem) =>
          String(problem).includes("run-artifact privacy finish line failed"),
        ),
        "digest-valid typed assurance evidence was omitted from the commissioned privacy closure",
      );
      assert(
        !JSON.stringify(referencedAssurancePrivacyBlocked).includes(
          assurancePrivateContact,
        ),
        "referenced-assurance privacy rejection disclosed private contact data",
      );
      ...
      await postJson(
        ...
        event(
          "artifact.written",
          "prove",
          "bridge proof revalidated after assurance-reference privacy rejection",
          { artifact: "proof/proof.json" },
        ),
      );
      ...
      await postJson(
        ...
        event(
          "artifact.written",
          "handoff",
          "reseal required handoff containing an adversarial secret fixture",
          { artifact: "handoff/final.md" },
        ),
      );
      const secretHandoffBlocked = await postJson(
        bridgePort,
        `/runs/${runId}/events`,
        event(
          "run.completed",
          "handoff",
          "reject required run artifact containing a secret",
          { artifact: "handoff/final.md" },
        ),
        409,
      );
      assert(
        (secretHandoffBlocked.problems || []).some((problem) =>
          String(problem).includes("run-artifact privacy finish line failed"),
        ),
        "bridge did not invoke the commissioned privacy gate for required run artifacts",
      );
      assert(
        !JSON.stringify(secretHandoffBlocked).includes(syntheticSecret),
        "bridge privacy rejection disclosed the detected secret",
      );
      ...
      await postJson(
        ...
        event(
          "artifact.written",
          "handoff",
          "bridge handoff restored after privacy rejection",
          { artifact: "handoff/final.md" },
        ),
      );
      await postJson(
        ...
        event(
          "artifact.written",
          "prove",
          "bridge proof revalidated after privacy rejection",
          { artifact: "proof/proof.json" },
        ),
      );
      ...
      const completed = await postJson(
        ...
        event(
          "run.completed",
          "handoff",
          "genuine commissioned v0.8 completion",
          { artifact: "handoff/final.md" },
        ),
      );
      assert(
        completed.run?.status === "complete",
        "genuine commissioned v0.8 bridge completion did not pass",
      );
      assert(
        completed.event?.type === "run.completed" &&
          completed.run.events?.some(({ type }) => type === "run.completed"),
        "successful commissioned completion was not persisted as a run.completed event",
      );
      ...
      assert(
        cacheAfter.completedRunValidation?.executions ===
          cacheBefore.completedRunValidation?.executions + 1 &&
          cacheAfter.completedRunValidation?.cacheHits >=
            cacheBefore.completedRunValidation?.cacheHits + 1,
        "unchanged completed-run polling reran finish-line gates instead of using the digest-bound validation cache",
      );

      writeFileSync(tracePath, traceWithPrivateContact, "utf8");
      writeJson(trajectoryPath, trajectoryWithUpdatedDigest);
      const driftedReferencedTrace = await getJson(
        bridgePort,
        `/runs/${runId}`,
        500,
      );
      assert(
        String(driftedReferencedTrace.error || "").includes(
          "run-artifact privacy finish line failed",
        ) &&
          !JSON.stringify(driftedReferencedTrace).includes(
            referencedPrivateContact,
          ),
        "completed-run read did not quarantine digest-valid referenced-trace privacy drift safely",
      );
      const driftedReferencedTraceList = await getJson(bridgePort, "/runs");
      assert(
        !driftedReferencedTraceList.some((entry) => entry.id === runId),
        "completed run with referenced-trace privacy drift remained visible in the trusted run list",
      );
      writeFileSync(tracePath, traceSource, "utf8");
      writeFileSync(trajectoryPath, trajectorySource, "utf8");
      const restoredReferencedTrace = await getJson(
        bridgePort,
        `/runs/${runId}`,
      );
      assert(
        restoredReferencedTrace.status === "complete",
        "completed run did not recover after referenced trajectory evidence was restored",
      );

      writeJson(evalPath, evalWithPrivateContact);
      const driftedCompleted = await getJson(bridgePort, `/runs/${runId}`, 500);
      assert(
        String(driftedCompleted.error || "").includes(
          "run-artifact privacy finish line failed",
        ) && !JSON.stringify(driftedCompleted).includes(privateContact),
        "completed-run read did not quarantine packet-bound privacy drift safely",
      );
      const driftedRunList = await getJson(bridgePort, "/runs");
      assert(
        !driftedRunList.some((entry) => entry.id === runId),
        "completed run with packet-bound privacy drift remained visible in the trusted run list",
      );
      writeFileSync(evalPath, evalSource, "utf8");
      const restoredCompleted = await getJson(bridgePort, `/runs/${runId}`);
      assert(
        restoredCompleted.status === "complete",
        "completed run did not recover after packet-bound privacy evidence was restored",
      );
      ...
      writeJson(aiAssurancePath, aiAssuranceWithPrivateReference);
      const driftedReferencedAssurance = await getJson(
        bridgePort,
        `/runs/${runId}`,
        500,
      );
      assert(
        String(driftedReferencedAssurance.error || "").includes(
          "run-artifact privacy finish line failed",
        ) &&
          !JSON.stringify(driftedReferencedAssurance).includes(
            assurancePrivateContact,
          ),
        "completed-run read did not quarantine digest-valid assurance-evidence privacy drift safely",
      );
      const driftedReferencedAssuranceList = await getJson(bridgePort, "/runs");
      assert(
        !driftedReferencedAssuranceList.some((entry) => entry.id === runId),
        "completed run with referenced assurance-evidence privacy drift remained visible in the trusted run list",
      );
      writeFileSync(aiAssurancePath, aiAssuranceSource, "utf8");
      rmSync(assuranceEvidencePath, { force: true });
      const restoredReferencedAssurance = await getJson(
        bridgePort,
        `/runs/${runId}`,
      );
      assert(
        restoredReferencedAssurance.status === "complete",
        "completed run did not recover after referenced assurance evidence was restored",
      );
      ...
      try {
        ...
        assert(
          typeof driftedRuntime.error === "string" &&
            driftedRuntime.error.length > 0,
          "completed-run cache ignored commissioned runtime drift",
        );
      } finally {
      ...
      assert(
        restoredRuntime.status === "complete",
        "completed run did not recover after commissioned runtime bytes were restored",
      );
    } finally {
      ...
      keys: [
        {
          ...
          algorithm: "ed25519",
          status: "active",
          publicKeyPem: attackerPublicKey.export({
            type: "spki",
            format: "pem",
          }),
    ...
    git(target, [
      "commit",
      "-m",
      "test: attacker self-enrolls commissioned review key",
    ]);
    const selfEnrollmentPort = bridgePort + 1;
    const selfEnrollmentBridge = trackedChild(
      spawn(
        process.execPath,
        [path.join(HARNESS_ROOT, "scripts", "claude-code-bridge.mjs")],
        {
    ...
    try {
      ...
      const rejectedSelfEnrollment = await postJson(
        selfEnrollmentPort,
        "/runs",
        {
      ...
      assert(
        (rejectedSelfEnrollment.problems || []).some((problem) =>
          String(problem).includes(

...
export function assertPairwiseDistinctBridgeCredentials(credentials) {
  const entries = BRIDGE_CREDENTIAL_ENV_NAMES.map((name) => [name, credentials[name]]);
  for (let leftIndex = 0; leftIndex < entries.length; leftIndex += 1) {
    for (let rightIndex = leftIndex + 1; rightIndex < entries.length; rightIndex += 1) {
      ...
      if (secretEqual(leftValue, rightValue)) {
        throw new Error(`bridge credentials must be pairwise distinct: ${leftName} and ${rightName} must not reuse a value`);
      }
    }
  }
}

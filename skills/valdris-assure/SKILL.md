...
3. For every non-docs route, produce `foundation/assessment.json` and run `foundation-gate.mjs`. Resolve product intent, requirements, architecture fit, repository understanding, ownership/risk, required skills, and the proof contract before implementation.

...
- Copy verified commissioned runtime bytes into a randomly rooted, owner-restricted execution capsule before probing. Remove POSIX directory write authority and apply an owner-only read/execute ACL to the Windows capsule and parent; launch only the capsule, revalidate its ACL, root identity, path, and bytes around every command, and bind the capsule path, root, and content digests into the receipt.

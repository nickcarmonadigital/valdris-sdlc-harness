...
6. For semantic or authoritative work, first bind the typed requirement, tools/calls, durable memory, runtime-driver/implementation, conditional judge-calibration, economics, interop, dependency-provenance, and trace-v2 artifacts, then run `run-create.mjs --print-accepted-gate-set` with every pre-closure gate and bind its digest into the executor and closure. Create `valdris.run-packet.v3` with `--assurance-level structural|semantic|authoritative`, the same gate/RCA arguments, and `--review review/review.json --output run/packet.json`. Semantic or authoritative packets also include `--gate authoritative-assurance=assurance/authoritative.json`. Run `node scripts/run-packet-gate.mjs --repo .`. Non-HEAD and genuine pre-v3 packets receive integrity-only structural inspection; full revalidation requires an isolated exact-commit checkout.

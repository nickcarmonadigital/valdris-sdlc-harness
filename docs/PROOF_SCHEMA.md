...
`run/packet.json` uses `valdris.run-packet.v2`. It exposes `roleProvenanceSha256`, validates that digest against the signed review, and includes it in the packet envelope binding. Its validation runtime records `reviewTrustSha256` and includes that value in the runtime `setSha256`, so the final packet is bound to the same external trust decision used for review. Its `generatedAt` must be canonical UTC ISO 8601 with millisecond precision and is also included in the envelope digest. Role collapse has no implicit tier exception; any future relaxation requires a versioned policy change plus an explicit governed waiver.

...
The final v0.8 packet uses `proof/portable.json` in addition to the bridge event proof above. `proof-runner.mjs` requires `--commit` to equal Git `HEAD`, binds the committed tree plus the exact dirty/untracked worktree and proof-runner digest before and after execution, rejects net source mutation that remains in the post-command snapshot, and preserves a separate application-source projection that excludes only the exact canonical post-proof artifacts `proof/portable.json`, `rca/rca.json`, `review/review.json`, and `run/packet.json`. Final packet validation recomputes that projection, so application code changed after proof is rejected; similarly named or noncanonical evidence files remain application source. The current pre/post snapshot model cannot prove that a command did not temporarily mutate application source and restore it before exit; immutable or read-only proof execution remains an open acceptance blocker. Local user paths and credential-like values are redacted; Windows `npm`, `pnpm`, and `yarn` shims resolve to `node` plus their JavaScript CLI while keeping `shell: false`.
...
`run/packet.json` uses `valdris.run-packet.v2`. It exposes `roleProvenanceSha256`, validates that digest against the signed review, and includes it in the packet envelope binding. New packets also carry a strictly sorted `artifactInventory` of repository-relative path, SHA-256, and size records for every non-review evidence file. The review evidence bundle signs that inventory and the packet envelope binds its digest. Hydrated CI acceptance requires this field, validates the exact transitive packet closure in a detached worktree, and touches the source checkout only after all gates pass. Historical v2 packets without the optional field remain readable but are not eligible for hydrated acceptance. The validation runtime records `reviewTrustSha256` and includes that value in the runtime `setSha256`, so the final packet is bound to the same external trust decision used for review. Its `generatedAt` must be canonical UTC ISO 8601 with millisecond precision and is also included in the envelope digest. Role collapse has no implicit tier exception; any future relaxation requires a versioned policy change plus an explicit governed waiver.
